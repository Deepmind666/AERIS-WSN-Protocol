...
class AerisProtocol:
    ...
    def __init__(self, config: NetworkConfig, *, enable_cas: bool = True, enable_fairness: bool = True, enable_aco_intercluster: bool = False, enable_gateway: bool = True, enable_skeleton: bool = True, profile: str | None = None, verbose: bool = True, seed: int | None = None, use_distilled_cas: Optional[bool] = None, mac_collision_model=None):
        self.config = config
    ...
    def _perform_data_transmission(self):
        ...
        # --- MAC collision factors ---
        mac = self.mac_collision_model
        num_chs = sum(1 for n in self.nodes if n.is_cluster_head and n.is_alive)
        _uplink_factor = mac.compute_uplink_factor(num_chs) if mac else 1.0
        ...
        def transmit_link(sender: Node, receiver: Node, tx_power_override: Optional[float] = None) -> bool:
            ...
            def attempt_link(tx_power: float) -> bool:
                ...
                # Apply intra-cluster collision factor from MAC model
                _intra_f = 1.0
                if mac and receiver.is_cluster_head:
                    _cs = getattr(receiver, 'cluster_size', 0) or 0
                    _intra_f = mac.compute_intra_factor(_cs)
                effective_pdr = link_metrics['pdr'] * _intra_f
                success = (self._rand.random() < effective_pdr)
                if success:
                    packets_received += 1
        ...
        def transmit_to_bs(
            ...
            def one_try_bs(bs_target: Optional[Tuple[float, float]] = None, tx_override: Optional[float] = None, sender_node: Optional[Node] = None) -> bool:
                ...
                ok = (self._rand.random() < link_metrics['pdr'] * _uplink_factor)
                if ok:
                    packets_received += 1
