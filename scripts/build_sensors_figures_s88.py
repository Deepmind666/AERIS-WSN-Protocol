...
def plot_fig6_s10() -> str:
    ...
    fig, axes = plt.subplots(3, 4, figsize=(16.6, 11.3), constrained_layout=True)
    ...
    for rr, comp in enumerate(comparisons):
        for cc, env in enumerate(ENV_ORDER):
            ...
            ax.set_xticklabels([str(n) for n in NODE_ORDER], fontsize=8.5)
            if rr == 2:
                ax.set_xlabel("Nodes")
            ax.set_yticks(np.arange(len(PROTOCOL_ORDER)))
            ax.set_yticklabels(PROTOCOL_ORDER, fontsize=8.0)
            if cc == 0:
            ...
            # Annotate every protocol-node cell so the map is audit-visible and cannot be misread as missing data.
            for r_idx in range(len(PROTOCOL_ORDER)):
                for c_idx in range(len(NODE_ORDER)):
                    val = matrix[r_idx, c_idx]
                    txt_color = "#1B2733" if abs(val) < 0.75 * vmax else "#10202F"
                    ax.text(
                        c_idx,
                        r_idx,
                        f"{val:+.2f}",
    ...
    fig.text(
        0.5,
        0.005,
        "Rows: pairwise tx comparisons. Columns: environments. Cells: protocol x node-count deltas (all cells annotated). Cross marker: Holm non-significant cell.",
        ha="center",
        va="bottom",
        fontsize=9.0,
        color="#4A5968",
...
def plot_fig7_ns3_trend() -> str:
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14.8, 9.6), constrained_layout=True)
    axes = axes.flatten()
