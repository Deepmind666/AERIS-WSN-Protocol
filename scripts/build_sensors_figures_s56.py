#!/usr/bin/env python3
"""
Sensors submission figures (S56): readability and camera-ready polish
for grayscale-safe printing and stricter panel legibility.
...
def plot_fig3() -> str:
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        for proto in PROTOCOL_ORDER:
            ...
            line_width = 3.4 if proto == "AERIS" else 2.65
            marker_size = 6.2 if proto == "AERIS" else 5.4
            # CI bands are retained for completeness; with n=3200 they are often narrower than line width.
            band_alpha = 0.18 if proto == "AERIS" else 0.10
            ax.plot(
                NODE_ORDER,
                means,
            ...
            ax.fill_between(NODE_ORDER, means - ci, means + ci, color=PROTO_COLORS[proto], alpha=band_alpha, linewidth=0)
        ...
        baseline_1000 = {p: curves[p][0][-1] for p in BASELINES}
        best_base = max(baseline_1000, key=baseline_1000.get)
        ax.text(
            0.02,
            0.08,
            f"AERIS@1000={curves['AERIS'][0][-1]:.3f}\n{best_base}@1000={baseline_1000[best_base]:.3f}",
            transform=ax.transAxes,
            ha="left",
            va="bottom",
            fontsize=7.8,
            color="#2F3A45",
            bbox={"facecolor": "#F8FBFF", "edgecolor": "#D1DAE3", "boxstyle": "round,pad=0.14"},
        )
        ax.text(
            0.98,
            0.06,
            f"v50 slope {aeris_slope:+.3f}",
            transform=ax.transAxes,
            ha="right",
            va="bottom",
            fontsize=7.5,
        ...
        # Add a compact inset for non-AERIS protocols to avoid low-range compression.
        from mpl_toolkits.axes_grid1.inset_locator import inset_axes

        inset = inset_axes(ax, width="44%", height="36%", loc="lower left", borderpad=1.1)
        non_aeris = [p for p in PROTOCOL_ORDER if p != "AERIS"]
        ymins = []
        ymaxs = []
        for proto in non_aeris:
            m, c = curves[proto]
            inset.plot(
                NODE_ORDER,
                m,
            ...
            ymins.append(float(np.min(m - c)))
            ymaxs.append(float(np.max(m + c)))
        y0 = min(ymins) - 0.015
        y1 = max(ymaxs) + 0.015
        inset.set_xlim(95, 1005)
        inset.set_ylim(max(0.0, y0), min(1.0, y1))
        inset.set_xticks([100, 500, 1000])
        inset.set_yticks(np.round(np.linspace(max(0.0, y0), min(1.0, y1), 3), 3))
        inset.tick_params(axis="both", labelsize=7.0)
        inset.grid(axis="both", alpha=0.28, linewidth=0.55)
        style_axes(inset)
        inset.text(
            0.98,
            0.08,
...
def plot_fig6_s10() -> str:
    ...
    fig = plt.figure(figsize=(13.3, 8.1), constrained_layout=True)
    gs = fig.add_gridspec(2, 3, width_ratios=[1.0, 1.0, 0.95], height_ratios=[1.0, 1.0], wspace=0.24, hspace=0.28)

    # Grayscale-safe map on |delta|, sign and significance are encoded by text/marker, not hue.
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        im = ax.imshow(np.abs(matrix), cmap=cmap, vmin=0.0, vmax=vmax, aspect="auto")
        ...
        for r in range(matrix.shape[0]):
            for c in range(matrix.shape[1]):
                val = matrix[r, c]
                label = f"{val:+.3f}"
                ax.text(c, r, label, ha="center", va="center", fontsize=7.0, color="#1F1F1F")
                if not sig_mask[r, c]:
                    ax.plot(c, r, marker="x", markersize=5.4, color="#383838", markeredgewidth=1.1, zorder=3)
                else:
                    ax.plot(c, r, marker="o", markersize=2.4, markerfacecolor="white", markeredgecolor="#2A2A2A", markeredgewidth=0.7, zorder=3)

    # shared colorbar
    cax = fig.add_axes([0.44, 0.06, 0.16, 0.02])
    cb = fig.colorbar(im, cax=cax, orientation="horizontal")
    cb.set_label(r"|Delta PDR| (tx5 - tx15)")
    ...
    hatches = {"AERIS": "//", "LEACH": "\\\\", "PEGASIS": "..", "HEED": "xx", "TEEN": "oo"}
    shades = {"AERIS": "#B8C6D5", "LEACH": "#CFD8E2", "PEGASIS": "#A7B9CB", "HEED": "#D8E0E8", "TEEN": "#9FB2C5"}
    for patch, proto in zip(bp["boxes"], PROTOCOL_ORDER):
        patch.set_facecolor(shades[proto])
        patch.set_alpha(0.95)
        patch.set_hatch(hatches[proto])
    ...
    ax.text(
        0.02,
        0.03,
        "x marker: Holm non-significant cell",
        transform=ax.transAxes,
        fontsize=7.3,
        color="#4A5968",
        bbox={"facecolor": "#F7FAFC", "edgecolor": "#D1DCE7", "boxstyle": "round,pad=0.12"},
    )
...
def plot_fig7_ns3_trend() -> str:
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        for x, yy, sig_ok, gval in zip(node_order, y, sigs, gs):
            if sig_ok:
                ax.plot(x, yy, marker="o", markersize=6.6, markerfacecolor="#4E79A7", markeredgecolor="#2B3948")
            else:
                ax.plot(x, yy, marker="x", markersize=7.2, color="#2B3948", markeredgewidth=1.25)
            if x in (100, 1000):
        ...
        ax.set_xticks(node_order)
        ax.set_xlabel("Number of nodes")
        ax.set_ylabel("AERIS - LEACH delta (percentage points)")
        ax.set_title(ENV_LABEL[env], pad=6)
        ax.grid(axis="both")
        style_axes(ax)
        panel_label(ax, f"({chr(97 + i)})")
...
def plot_fig8_s8_significance_heatmap() -> str:
    ...
    # Use log-scaled effect-size magnitude to avoid saturation from a few extreme cells.
    gabs = np.abs(gvals)
    gcolor = np.log10(1.0 + gabs)
    cmap_b = LinearSegmentedColormap.from_list("sig_soft", ["#F3F6FB", "#D7E7F4", "#9FC3E0", "#5E8FB6"])
    im1 = axes[1].imshow(gcolor, aspect="auto", cmap=cmap_b)
