...
$tag = 'suburban_urban'
$outDirWin = Join-Path $projectWin "ns3_validation\results\lcn26_ns3_ablation_fat_${tag}_$stamp"
$outDirWsl = "$projectWsl/ns3_validation/results/lcn26_ns3_ablation_fat_${tag}_$stamp"
$logDirWin = Join-Path $outDirWin 'logs'
$stdoutLog = Join-Path $logDirWin 'ablation_fat.stdout.log'
$stderrLog = Join-Path $logDirWin 'ablation_fat.stderr.log'
...
$runScriptWin = Join-Path $outDirWin 'run_ablation_fat.sh'
$runScriptWsl = "$outDirWsl/run_ablation_fat.sh"
...
Write-Host "[STARTED] FatMachine ablation NS-3 PID: $($proc.Id)"
