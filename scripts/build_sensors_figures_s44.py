#!/usr/bin/env python3
"""
Sensors submission figures (S44): expanded evidence utilization with
three additional figures for significance, stress consistency, and
absolute tx-power behavior.

Outputs:
- fig0_aeris_workflow_20260221_s44.{pdf,svg,png}
- fig1_env_pdr_panel_20260221_s44.{pdf,svg,png}
- fig2_ablation_panel_20260221_s44.{pdf,svg,png}
- fig3_scalability_panel_20260221_s44.{pdf,svg,png}
- fig4_tradeoff_panel_20260221_s44.{pdf,svg,png}
- fig5_s11_patch_control_delta_20260221_s44.{pdf,svg,png}
- fig6_s10_power_sensitivity_20260221_s44.{pdf,svg,png}
- fig7_ns3_trend_panel_20260221_s44.{pdf,svg,png}
- fig8_s8_significance_heatmap_20260221_s44.{pdf,svg,png}
- fig9_s9_s11_consistency_20260221_s44.{pdf,svg,png}
- fig10_s10_absolute_profiles_20260221_s44.{pdf,svg,png}
"""
...
def plot_fig0_workflow() -> str:
    ...
    ax.text(0.5, 6.55, "AERIS round-level workflow and evidence pipeline", fontsize=12.0, fontweight="bold", color="#1F2A35")
...
def plot_fig3() -> str:
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        for proto in PROTOCOL_ORDER:
            ...
            line_width = 3.2 if proto == "AERIS" else 2.25
            marker_size = 6.0 if proto == "AERIS" else 4.9
            band_alpha = 0.24 if proto == "AERIS" else 0.14
            ax.plot(
                NODE_ORDER,
                means,
...
def plot_fig8_s8_significance_heatmap() -> str:
    rows = [r for r in load_csv(S10_SIG_FILE) if r["protocol"] == "AERIS"]
    # Reuse style on a true S8 significance matrix.
    s8_rows = [r for r in load_csv(RESULTS_DIR / "scalability_4env_s8_unified_20260215_significance.csv") if r["baseline"] == "TEEN"]
    ...
    for i, env in enumerate(ENV_ORDER):
        for j, n in enumerate(NODE_ORDER):
            row = [x for x in s8_rows if x["environment"] == env and int(x["num_nodes"]) == n][0]
    ...
    cmap_a = LinearSegmentedColormap.from_list("delta_soft", ["#EED8C8", "#F7F8FA", "#D6E6F3"])
    im0 = axes[0].imshow(diff, aspect="auto", cmap=cmap_a)
    ...
    axes[0].set_title("S8 delta to strongest baseline (pp)")
    for i in range(len(ENV_ORDER)):
        for j in range(len(NODE_ORDER)):
            axes[0].text(j, i, f"{diff[i,j]:+.2f}", ha="center", va="center", fontsize=7.4, color="#2A2A2A")
    panel_label(axes[0], "(a)")
    style_axes(axes[0])
    cb0 = fig.colorbar(im0, ax=axes[0], shrink=0.84, pad=0.02)
    cb0.set_label("Delta PDR (pp)")

    # -log10 Holm p for significance strength.
    ...
    im1 = axes[1].imshow(neglogp, aspect="auto", cmap=cmap_b)
    ...
    axes[1].set_title("S8 significance strength (-log10 Holm p)")
    for i in range(len(ENV_ORDER)):
        for j in range(len(NODE_ORDER)):
            mark = "Y" if sigmask[i, j] else "N"
            axes[1].text(j, i, f"{mark}\ng={gvals[i,j]:.1f}", ha="center", va="center", fontsize=7.0, color="#1F2A35")
    panel_label(axes[1], "(b)")
    style_axes(axes[1])
    cb1 = fig.colorbar(im1, ax=axes[1], shrink=0.84, pad=0.02)
    cb1.set_label("-log10 p_holm")
...
def main() -> None:
    ...
    for stem in [fig0, fig1, fig2, fig3, fig4, fig5, fig6, fig7, fig8, fig9, fig10]:
