...
def plot_fig7_ns3_trend() -> str:
    ...
    primary_handles = [
        plt.Line2D(
            ...
            marker=PROTO_MARKERS[p],
            linestyle=PROTO_LINESTYLES[p],
            linewidth=2.8 if p == "AERIS" else 2.4,
            markersize=5.8,
            label=p,
        )
        for p in ("AERIS", "LEACH")
    ]
    ...
    leg1 = fig.legend(
        ...
        framealpha=0.95,
        edgecolor="#C5D0DB",
        title="Primary (inferential)",
    )
    leg2 = fig.legend(
        handles=context_handles,
        loc="lower center",
        ncol=3,
        bbox_to_anchor=(0.77, -0.03),
        frameon=True,
        framealpha=0.95,
        edgecolor="#C5D0DB",
        title="Context (descriptive)",
    )
    fig.add_artist(leg1)
    fig.add_artist(leg2)
