...
PROTO_COLORS = {
    "AERIS": "#5F8FB8",   # muted blue
    "LEACH": "#D4895A",   # muted orange
    "PEGASIS": "#74A89A", # muted teal
    "HEED": "#AF8CB2",    # muted mauve
    "TEEN": "#CFA44D",    # muted amber
}
...
def plot_figure4_tradeoff() -> str:
    ...
    for idx, (ax, (metric, xlabel, desc)) in enumerate(zip(flat_axes, panels)):
        ...
        ax.barh(y, ranked_vals, color=[PROTO_COLORS[p] for p in ranked_proto], edgecolor="#57616D", linewidth=0.8)
        style_axes(ax)
        ax.set_yticks(y)
