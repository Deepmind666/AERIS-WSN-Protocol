...
def plot_fig6_s10() -> str:
    ...
    fig, axes = plt.subplots(3, 4, figsize=(18.2, 12.2), constrained_layout=True)
    ...
    for rr, comp in enumerate(comparisons):
        for cc, env in enumerate(ENV_ORDER):
            ...
            # Numeric annotations are intentionally removed to keep cells readable in print.
    ...
    fig.text(
        0.5,
        0.005,
        "Rows: pairwise tx comparisons. Columns: environments. Cells: protocol x node-count deltas (full coverage). Cross marker: Holm non-significant cell.",
        ha="center",
        va="bottom",
        fontsize=9.8,
        color="#4A5968",
...
def plot_fig8_s8_significance_heatmap() -> str:
    ...
    # Use Holm significance strength and clip at a fixed cap to avoid full saturation.
    p_floor = 1e-300
    ...
    cb1.set_label(r"$-\log_{10}(\mathrm{Holm}\ p)$ (clipped at 30)")
