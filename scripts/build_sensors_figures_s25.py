...
PROTO_COLORS = {
    "AERIS": "#4F7EA8",   # muted steel blue
    "LEACH": "#CF9368",   # soft clay orange
    "PEGASIS": "#69A88F", # muted sea green
    "HEED": "#B08BB6",    # muted mauve
    "TEEN": "#C7A862",    # muted ochre
}
...
def plot_figure1_env_pdr() -> str:
    ...
    for idx, env in enumerate(ENV_ORDER):
        ...
        draw_rounded_bars(
            ...
            heights=means,
            errors=stds,
            colors=[PROTO_COLORS[p] for p in PROTOCOL_ORDER],
            width=0.74,
        )
...
def plot_figure2_ablation() -> str:
    ...
    axes[1].hlines(y + 0.14, 0, gw_delta, color="#BC6D46", linewidth=2.4, label="no_gateway - full")
    axes[1].hlines(y - 0.14, 0, cas_delta, color="#4F7EA8", linewidth=2.4, label="no_cas - full")
    axes[1].plot(gw_delta, y + 0.14, "o", color="#BC6D46", markersize=5.4)
    axes[1].plot(cas_delta, y - 0.14, "s", color="#4F7EA8", markersize=5.4)
    ...
    for yi, val in zip(y + 0.14, gw_delta):
        ...
        axes[1].text(val + axoff, yi, f"{val:+.2f}", va="center", ha=ha, fontsize=8.1, color="#7A4A31")
    for yi, val in zip(y - 0.14, cas_delta):
        axoff = 0.08 if val >= 0 else -0.08
        ha = "left" if val >= 0 else "right"
        axes[1].text(val + axoff, yi, f"{val:+.2f}", va="center", ha=ha, fontsize=8.1, color="#335E84")
...
def plot_figure3_scalability(desc_csv: Path) -> str:
    ...
    for idx, env in enumerate(ENV_ORDER):
        ...
        for proto in PROTOCOL_ORDER:
            ...
            ax.fill_between(NODE_ORDER, means - ci, means + ci, color=PROTO_COLORS[proto], alpha=0.14, linewidth=0)
        panel_label(ax, f"({chr(97 + idx)})")
        style_axes(ax)
        ax.set_title(ENV_LABEL[env], pad=8)
        ...
        ax.grid(axis="both", alpha=0.2, linewidth=0.8)
