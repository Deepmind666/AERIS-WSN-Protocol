    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        if env == "outdoor_urban":
            ...
            draw_rounded_bars(
                ...
                means,
                stds,
                [PROTO_COLORS[p] for p in PROTOCOL_ORDER],
            )
            inset.set_ylim(0.0, 0.16)
            inset.set_xticks(x)
            inset.set_xticklabels(["A", "L", "P", "H", "T"], fontsize=8.2)
            inset.set_yticks([0.00, 0.08, 0.16])
            inset.tick_params(labelsize=8.0)
            inset.grid(axis="y", alpha=0.30)
            inset.set_title("low-range zoom", fontsize=9.0, pad=2)
...
def plot_fig3() -> str:
    ...
    fig.text(
        0.5,
        0.015,
        "95% CI bands are rendered for all protocols; with n=3200 most bands are narrower than line width.",
        ha="center",
        fontsize=9.1,
        color="#4A5968",
    )
...
def plot_fig6_s10() -> str:
    ...
    # Convert absolute PDR deltas into percentage points to match axis units.
    delta_pp = {(r["environment"], int(r["num_nodes"]), r["protocol"]): float(r["delta"]) * 100.0 for r in rows}
    sig = {(r["environment"], int(r["num_nodes"]), r["protocol"]): (r["significant_005"] == "YES") for r in rows}

    vabs = np.array([abs(v) for v in delta_pp.values()], dtype=float)
    vmax = float(max(5.0, np.quantile(vabs, 0.97)))
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        for r, proto in enumerate(PROTOCOL_ORDER):
            for c, n in enumerate(NODE_ORDER):
                matrix[r, c] = delta_pp[(env, n, proto)]
        ...
        for r in range(matrix.shape[0]):
            for c in range(matrix.shape[1]):
                val = matrix[r, c]
                ax.text(c, r, f"{val:+.1f}", ha="center", va="center", fontsize=8.6, color="#1F1F1F")
                if not sig_mask[r, c]:
    ...
    cb.set_label(r"Delta PDR (tx5 - tx15, pp)")
    fig.text(
        0.5,
        0.014,
        "All 4 environments, 5 protocols, and 6 node scales shown (120 cells). x marker: Holm non-significant cell",
        ha="center",
        fontsize=8.0,
        color="#4A5968",
    )
...
def plot_fig7_ns3_trend() -> str:
    ...
    axes[0].text(
        0.02,
        0.08,
        "red hollow circle: Holm non-significant (AERIS vs LEACH only)",
...
def plot_fig10_s10_absolute_profiles() -> str:
    ...
    fig, axes = plt.subplots(2, 4, figsize=(15.2, 9.4), constrained_layout=True)
    tx_styles = {
        5.0: "--",
        10.0: ":",
        15.0: "-",
    ...
    fig.text(
        0.5,
        0.015,
        "Top row uses full y-range [0,1]. Baseline row uses environment-specific zoomed y-range for readability.",
        ha="center",
        fontsize=9.1,
        color="#4A5968",
    )
