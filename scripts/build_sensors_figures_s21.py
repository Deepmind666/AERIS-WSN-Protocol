...
PROTO_COLORS = {
    "AERIS": "#5A84A8",   # soft blue-gray
    "LEACH": "#D29A6A",   # soft orange
    "PEGASIS": "#7AAE9D", # soft teal
    "HEED": "#B58EB9",    # soft mauve
    "TEEN": "#CEAC64",    # soft amber
}
...
PRECOMP_DESC = RESULTS_DIR / "scalability_4env_mixed_20260213_s11_descriptive.csv"
PRECOMP_SIG = RESULTS_DIR / "scalability_4env_mixed_20260213_s11_significance.csv"
...
def plot_figure4_tradeoff() -> str:
    ...
    for idx, (ax, (metric, xlabel, desc)) in enumerate(zip(flat_axes, panels)):
        ...
        x0 = 1.0 if metric == "hops" else 0.0
        for yi, proto, vv in zip(y, ranked_proto, ranked_vals):
            ax.hlines(yi, x0, vv, color=PROTO_COLORS[proto], linewidth=4.0, alpha=0.92, zorder=2)
            ax.scatter(vv, yi, s=46, color=PROTO_COLORS[proto], edgecolors="#52606E", linewidths=0.6, zorder=3)
        style_axes(ax)
        ax.set_yticks(y)
...
def main() -> None:
    apply_style()
    # Use the previously audited scalability matrix for interim drafting
    # until the full S8 unified rebuild is completed.
    if PRECOMP_DESC.exists() and PRECOMP_SIG.exists():
        desc_csv = PRECOMP_DESC
        sig_csv = PRECOMP_SIG
        md_out = RESULTS_DIR / f"scalability_4env_mixed_{SUFFIX}_summary.md"
        md_out.write_text(
            "# Scalability Summary (S21)\n\n"
            "- source: precomputed audited matrix `scalability_4env_mixed_20260213_s11_*`\n",
            encoding="utf-8",
        )
    else:
