...
Write-Output ("[{0}] QUEUE_START tasks={1} policy=max_cpu<=88 max_mem<=92 workers=16" -f ($globalStart.ToString('yyyy-MM-dd HH:mm:ss')), $tasks.Count)

for ($i = 0; $i -lt $tasks.Count; $i++) {
    $t = $tasks[$i]
    $taskIdx = $i + 1
    $etaCurrent = "~120-150min"
    $etaGlobal = "~{0}-{1}min" -f (120 * ($tasks.Count - $i)), (150 * ($tasks.Count - $i))
    Write-Output ("[{0}] ETA task={1}/{2} env={3} tx={4} eta_current={5} eta_global={6} basis=recent_local_s10r_2.1h_per_task" -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'), $taskIdx, $tasks.Count, $t.Env, $t.Tx, $etaCurrent, $etaGlobal)
