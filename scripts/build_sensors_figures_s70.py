...
def plot_fig6_s10() -> str:
    ...
    vmax = float(max(0.02, np.quantile(vabs, 0.97)))
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        for r in range(matrix.shape[0]):
            for c in range(matrix.shape[1]):
                val = matrix[r, c]
                ax.text(c, r, f"{val:+.3f}", ha="center", va="center", fontsize=7.0, color="#1F1F1F")
                if not sig_mask[r, c]:
...
def plot_fig7_ns3_trend() -> str:
    ...
    axes[0].text(
        0.02,
        0.08,
        "x marker: Holm non-significant (AERIS vs LEACH only)",
...
def plot_fig10_s10_absolute_profiles() -> str:
        ...
        for proto in PROTOCOL_ORDER:
            for tx in [5.0, 10.0, 15.0]:
                ...
                ax.plot(
                    ...
                    marker=PROTO_MARKERS[proto],
                    linewidth=2.2 if tx == 15.0 else 1.75,
                    markersize=4.4 if tx == 15.0 else 3.6,
                    color=PROTO_COLORS[proto],
                    alpha=0.95 if tx == 15.0 else 0.75,
                )
    ...
    l2, = axes[0].plot([], [], color="#4F4F4F", linestyle=":", linewidth=2.1, label="tx10")
    l3, = axes[0].plot([], [], color="#4F4F4F", linestyle="--", linewidth=2.1, label="tx5")
    ...
    fig.legend(handles=[l1, l2, l3] + handles_proto, loc="lower center", ncol=8, bbox_to_anchor=(0.5, -0.01), frameon=True, framealpha=0.94, edgecolor="#C5D0DB")
