...
def plot_fig1() -> str:
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        if env == "outdoor_urban":
            ...
            inset.set_xticks(x)
            inset.set_xticklabels(["A", "L", "P", "H", "T"], fontsize=8.4)
            inset.set_yticks([0.00, 0.08, 0.16])
            inset.tick_params(labelsize=8.2)
            inset.grid(axis="y", alpha=0.30)
            inset.set_title("Inset: 0-0.16 PDR", fontsize=9.4, pad=2)
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        for r in range(matrix.shape[0]):
            for c in range(matrix.shape[1]):
                val = matrix[r, c]
                ax.text(c, r, f"{val:+.1f}", ha="center", va="center", fontsize=9.1, color="#1F1F1F")
                if not sig_mask[r, c]:
