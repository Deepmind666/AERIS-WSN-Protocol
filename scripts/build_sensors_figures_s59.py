#!/usr/bin/env python3
"""
Sensors submission figures (S59): readability and camera-ready polish
with colorblind-safe palettes and higher density for core result panels.
...
def plot_fig3() -> str:
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        for proto in PROTOCOL_ORDER:
            ...
            band_alpha = 0.15 if proto == "AERIS" else 0.08
            ax.plot(
                NODE_ORDER,
                means,
        ...
        # Adaptive limits improve local readability while preserving absolute scale cues.
        all_vals = np.concatenate([curves[p][0] for p in PROTOCOL_ORDER])
        y_lo = max(0.0, float(np.min(all_vals)) - 0.04)
        y_hi = min(1.0, float(np.max(all_vals)) + 0.03)
        if y_hi - y_lo < 0.25:
            mid = 0.5 * (y_hi + y_lo)
            y_lo = max(0.0, mid - 0.125)
            y_hi = min(1.0, mid + 0.125)
        ...
        inset = inset_axes(ax, width="52%", height="42%", loc="lower left", borderpad=1.0)
        ...
        inset.tick_params(axis="both", labelsize=7.2)
        ...
        if env == "outdoor_urban":
            ax.annotate(
                "steep degradation zone",
                xy=(820, curves["AERIS"][0][4]),
                xytext=(610, curves["AERIS"][0][4] + 0.18),
                fontsize=7.6,
                color="#6A3A31",
                arrowprops=dict(arrowstyle="->", color="#6A3A31", lw=0.9),
                bbox={"facecolor": "#FFF7F4", "edgecolor": "#E9C8BF", "boxstyle": "round,pad=0.15"},
            )
