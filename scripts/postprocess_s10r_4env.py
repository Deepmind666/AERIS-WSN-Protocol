...
Outputs:
  - s10r_4env_merged_descriptive_20260226.csv
  - s10r_4env_significance_tx5_vs_tx10_vs_tx15_20260226.csv
  - s10r_4env_reconciliation_20260226.md
"""
...
# Expected S10R sources (local + server split).
FILES: Dict[Tuple[str, float], str] = {
    ...
    ("indoor_factory", 5.0): "scalability_indoor_factory_local_s10r_tx5_20260226.json",
    ("indoor_factory", 10.0): "scalability_indoor_factory_local_s10r_tx10_20260226.json",
    ("indoor_factory", 15.0): "scalability_indoor_factory_local_s10r_tx15_20260226.json",
    ...
    ("outdoor_suburban", 5.0): "scalability_outdoor_suburban_server_s10r_tx5_20260226.json",
    ("outdoor_suburban", 10.0): "scalability_outdoor_suburban_server_s10r_tx10_20260226.json",
    ("outdoor_suburban", 15.0): "scalability_outdoor_suburban_server_s10r_tx15_20260226.json",
}
...
def extract_pdr(rows: Iterable[dict], env: str, nodes: int, proto: str) -> List[float]:
    ...
    for r in rows:
        ...
        if r.get("error"):
            continue
        if "pdr_expected" in r and r["pdr_expected"] is not None:
            out.append(float(r["pdr_expected"]))
    return out
...
def main() -> None:
    ...
    # 1) descriptive
    desc_rows: List[dict] = []
    for env in sorted({k[0] for k in FILES.keys()}):
    ...
    desc_csv = BASE / "s10r_4env_merged_descriptive_20260226.csv"
    ...
    # 2) significance pairwise across tx
    sig_rows: List[dict] = []
    for env in sorted({k[0] for k in FILES.keys()}):
        for nn in NODES:
            for proto in PROTOCOLS:
    ...
    sig_csv = BASE / "s10r_4env_significance_tx5_vs_tx10_vs_tx15_20260226.csv"
    ...
    report = BASE / "s10r_4env_reconciliation_20260226.md"
    with report.open("w", encoding="utf-8") as f:
        f.write("# S10R 4-Environment Reconciliation (20260226)\n\n")
