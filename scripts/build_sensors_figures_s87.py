...
def plot_fig6_s10() -> str:
    ...
    for rr, comp in enumerate(comparisons):
        for cc, env in enumerate(ENV_ORDER):
            ...
            ax.set_yticklabels(PROTOCOL_ORDER if cc == 0 else [], fontsize=8.8)
            if cc == 0:
    ...
    fig.text(
        0.5,
        0.005,
        "Rows: pairwise tx comparisons. Columns: environments. Cells: protocol x node-count deltas. Cross marker: Holm non-significant cell.",
        ha="center",
        va="bottom",
        fontsize=9.0,
        color="#4A5968",
