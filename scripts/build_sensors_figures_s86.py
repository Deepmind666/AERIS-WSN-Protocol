...
def plot_fig6_s10() -> str:
    ...
    vmax = float(max(0.08, np.quantile(vabs, 0.98)))

    fig, axes = plt.subplots(3, 4, figsize=(16.0, 10.8), constrained_layout=True)
    ...
    for rr, comp in enumerate(comparisons):
        for cc, env in enumerate(ENV_ORDER):
            ...
            ax.set_xticklabels([str(n) for n in NODE_ORDER], fontsize=8.6)
            if rr == 2:
                ax.set_xlabel("Nodes")
            if cc == 0:
                ax.set_yticks(np.arange(len(PROTOCOL_ORDER)))
                ax.set_yticklabels(PROTOCOL_ORDER, fontsize=8.8)
                ax.set_ylabel(cmp_label[comp], fontsize=9.4)
            else:
                ax.set_yticks(np.arange(len(PROTOCOL_ORDER)))
                ax.set_yticklabels([])
            ...
            # Show non-significant cells only; avoid dense text clutter.
            ...
            # Add numeric labels only for AERIS row to keep readability while exposing absolute magnitudes.
            aeris_row = 0
            for c_idx in range(len(NODE_ORDER)):
                val = matrix[aeris_row, c_idx]
                ax.text(
                    c_idx,
                    aeris_row,
                    f"{val:+.3f}",
    ...
    fig.text(
        0.5,
        0.005,
        "Rows: pairwise tx comparisons. Columns: environments. Cross marker: Holm non-significant cell.",
        ha="center",
        va="bottom",
        fontsize=9.0,
        color="#4A5968",
