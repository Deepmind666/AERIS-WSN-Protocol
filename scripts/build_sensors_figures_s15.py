...
PROTO_COLORS = {
    "AERIS": "#4E79A7",   # muted blue
    "LEACH": "#D18B47",   # soft orange
    "PEGASIS": "#76B7B2", # muted teal
    "HEED": "#B07AA1",    # soft mauve
    "TEEN": "#E0AC2B",    # soft amber
}
...
def plot_figure1_env_pdr() -> str:
    ...
    for idx, env in enumerate(ENV_ORDER):
        ...
        draw_rounded_bars(ax, x, means, stds, [PROTO_COLORS[p] for p in PROTOCOL_ORDER], width=0.8)
...
def plot_figure2_ablation() -> str:
    ...
    im = axes[0].imshow(matrix, cmap="Blues", vmin=0, vmax=1, aspect="auto")
    ...
    for idx, (ax, (metric, xlabel, desc)) in enumerate(zip(flat_axes, panels)):
        ...
        for yi, vv in zip(y, ranked_vals):
            if metric == "life":
                ...
                label = f"{vv:.3f}"
            ax.text(vv + vmax * 0.008, yi, label, va="center", ha="left", fontsize=8)

        # In hop-latency ranking, PEGASIS dominates the x-range.
        # Add a zoomed inset so low-hop protocols remain visually comparable.
        if metric == "hops":
            non_outlier = [v for p, v in zip(ranked_proto, ranked_vals) if p != "PEGASIS"]
            zoom_max = max(non_outlier) * 1.35 if non_outlier else vmax
            zoom_ax = inset_axes(ax, width="52%", height="58%", loc="lower right", borderpad=1.1)
            zoom_ax.barh(y, ranked_vals, color=[PROTO_COLORS[p] for p in ranked_proto], edgecolor="black", linewidth=0.45)
            zoom_ax.set_xlim(0.0, zoom_max)
            zoom_ax.invert_yaxis()
            zoom_ax.set_yticks(y)
            zoom_ax.set_yticklabels(ranked_proto, fontsize=6.7)
            zoom_ax.tick_params(axis="x", labelsize=6.5, length=2)
            zoom_ax.grid(axis="x", alpha=0.13)
            for side in ("top", "right"):
                zoom_ax.spines[side].set_visible(False)
            zoom_ax.spines["left"].set_linewidth(0.6)
            zoom_ax.spines["bottom"].set_linewidth(0.6)
            zoom_ax.set_title("zoom (non-PEGASIS)", fontsize=6.8, pad=2)
