...
def plot_fig1() -> str:
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        if env == "outdoor_urban":
            # Show baseline separation only (exclude AERIS) to avoid a duplicate-looking inset.
            ax.axhspan(0.0, 0.16, facecolor="#EEF5FB", alpha=0.26, zorder=0)
            inset = ax.inset_axes([0.52, 0.48, 0.43, 0.48])
            b_x = np.arange(len(BASELINES))
            b_means = [stats_map[(env, p)][0] for p in BASELINES]
            b_stds = [stats_map[(env, p)][1] for p in BASELINES]
            draw_rounded_bars(
                inset,
                b_x,
                b_means,
                b_stds,
                [PROTO_COLORS[p] for p in BASELINES],
            )
            inset.set_ylim(0.0, 0.16)
            inset.set_xticks(b_x)
            inset.set_xticklabels(["L", "P", "H", "T"], fontsize=8.6)
            inset.set_yticks([0.00, 0.08, 0.16])
            inset.tick_params(labelsize=8.4)
            inset.grid(axis="y", alpha=0.30)
            inset.set_title("Baseline zoom (0-0.16)", fontsize=9.6, pad=2)
            for spine in inset.spines.values():
                spine.set_color("#8FA1B2")
...
def plot_fig3() -> str:
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        # Baseline inset to keep low-PDR protocols readable under common 0-1 main axis.
        b_vals = {
            proto: np.array([stats_map[(env, n, proto)][0] for n in NODE_ORDER], dtype=float)
            for proto in BASELINES
        }
        b_min = min(float(np.min(v)) for v in b_vals.values())
        b_max = max(float(np.max(v)) for v in b_vals.values())
        y0 = max(0.0, b_min - 0.03)
        y1 = min(1.0, b_max + 0.03)
        if y1 - y0 > 0.04:
            inset = ax.inset_axes([0.56, 0.05, 0.40, 0.34])
            for proto in BASELINES:
                inset.plot(
                    NODE_ORDER,
                    b_vals[proto],
            ...
            inset.tick_params(labelsize=8.2)
            inset.grid(axis="both", alpha=0.22)
            inset.set_title("Baseline zoom", fontsize=8.8, pad=2)
            for spine in inset.spines.values():
                spine.set_color("#8FA1B2")
                spine.set_linewidth(0.65)
...
def plot_fig6_s10() -> str:
    rows = [r for r in load_csv(S10_SIG_FILE) if r["comparison"] == "tx5_vs_tx15"]

    # Keep absolute delta units (0-1 PDR range) to avoid unit ambiguity.
    delta_abs = {(r["environment"], int(r["num_nodes"]), r["protocol"]): float(r["delta"]) for r in rows}
    ...
    vmax = float(max(0.08, np.quantile(vabs, 0.97)))
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        for r, proto in enumerate(PROTOCOL_ORDER):
            for c, n in enumerate(NODE_ORDER):
                matrix[r, c] = delta_abs[(env, n, proto)]
        ...
        for r in range(matrix.shape[0]):
            for c in range(matrix.shape[1]):
                val = matrix[r, c]
                ax.text(c, r, f"{val:+.3f}", ha="center", va="center", fontsize=9.1, color="#1F1F1F")
                if not sig_mask[r, c]:
                    ax.plot(c, r, marker="x", markersize=5.2, color="#2F2F2F", markeredgewidth=1.1, zorder=3)

    cb = fig.colorbar(im, ax=axes, shrink=0.84, pad=0.02)
    cb.set_label(r"Delta PDR (tx5 - tx15, absolute)")
...
def plot_fig7_ns3_trend() -> str:
    ...
    for i, env in enumerate(ENV_ORDER):
        ...
        for proto in PROTOCOL_ORDER:
            series = [desc_map[(env, n, proto)] for n in node_order]
            if proto in ("AERIS", "LEACH"):
                ax.plot(
                    ...
                    color=PROTO_COLORS[proto],
                    marker=PROTO_MARKERS[proto],
                    linewidth=2.8 if proto == "AERIS" else 2.7,
                    markersize=7.0 if proto == "AERIS" else 6.8,
                    label=f"{proto} (primary)",
                    zorder=3 if proto == "AERIS" else 2,
                )
            else:
                ax.plot(
                    node_order,
...
def plot_fig8_s8_significance_heatmap() -> str:
    ...
    p_cap = float(np.quantile(p_strength, 0.80))
...
def plot_fig10_s10_absolute_profiles() -> str:
    ...
    fig, axes = plt.subplots(2, 4, figsize=(16.0, 10.2), constrained_layout=True)
    tx_styles = {
        5.0: "--",
        10.0: ":",
        15.0: "-",
