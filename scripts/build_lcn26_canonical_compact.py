#!/usr/bin/env python3
"""Build a compact single-column canonical NS-3 figure for the LCN draft."""
...
def build() -> None:
    data = load_data()
    fig, axes = plt.subplots(2, 2, figsize=(3.52, 3.56), sharex=True, sharey=True)
    axes = axes.flatten()
    ...
    fig.legend(handles=handles, labels=PROTO_ORDER, ncol=5, loc="upper center", bbox_to_anchor=(0.5, 1.00), frameon=False, columnspacing=0.52, handletextpad=0.22)
    fig.subplots_adjust(top=0.79, left=0.12, right=0.98, bottom=0.14, wspace=0.16, hspace=0.30)
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    fig.savefig(OUT_DIR / "fig_lcn26_ns3_canonical_compact.pdf")
    fig.savefig(OUT_DIR / "fig_lcn26_ns3_canonical_compact.png")
    plt.close(fig)
