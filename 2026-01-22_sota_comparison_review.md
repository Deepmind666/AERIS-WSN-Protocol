...
## 1. 本次修正目标
解决 Figure 6-panel 中 (c) 子图意义不清/点数过少、(d) 过度散点导致审稿质疑、(f) 表格字体过小、以及整体可读性不足的问题。
...
### 2.2 (d) 改为 Mean ± 95% CI 的 PDR–Energy Trade-off
- 取消散点云，改为每协议一个均值点 + 双向 CI。
- 直观看到“高 PDR 伴随高能耗”的 trade-off，避免结论被质疑。

### 2.3 (f) 表格可读性提升
- 显著增大字号与表格缩放比例，扩展表格占用区域。
...
## 5. 结论
(c) 子图已从“抽象点状效应量”改为“均值+置信区间+差值标注”；(d) 子图改为均值+CI 的 trade-off 展示；(f) 表格可读性提升。当前版本可满足审稿人可解释性要求。
