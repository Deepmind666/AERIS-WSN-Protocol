...
The ablation uses the same seed structure and environment-scale grid, but compares full AERIS with one disabled component at a time. The reported contribution is full-minus-ablated PDR in percentage points. Statistical support is computed per environment across the seven node scales after Holm correction. This convention is deliberately conservative: it avoids using the 3360-run count as if every packet or round were an independent statistical sample.

The five-protocol NS-3 comparison anchors the classical-baseline ranking; the seven-protocol sweep maps the boundary against simplified collection-style baselines; the ablation attributes the gain; and the collision-aware stress layer provides stress evidence rather than a second canonical ranking. Lifetime, FND, hops, and energy expose load concentration. In Fig.~\ref{fig:ablation}, the module-level bar heights summarize Welch-type cell tests with Hedges' $g$ and Holm correction. In Fig.~\ref{fig:ns3_boundary}, winner labels alone should not be read as significance claims.
...
\caption{Expanded NS-3 AERIS ablation across four environments and 50--1000 nodes ($n=30$ per cell). The grouped bars report the mean full-minus-ablated PDR contribution of each removed module by environment; whiskers show the observed node-scale range.}
\label{fig:ablation}
\end{figure}
