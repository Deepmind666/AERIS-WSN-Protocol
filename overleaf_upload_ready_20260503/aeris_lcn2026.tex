...
$(\hat{E}_i,\hat{L}_i,\hat{D}_{iB},\hat{C}_i,\hat{\rho}_i,\hat{U}_i,p_i)$.
...
\begin{table}[t]
\caption{AERIS state variables and their protocol roles.}
\label{tbl:aeris_state}
\centering
\scriptsize
...
\midrule
$\hat{E}_i$ & residual energy & CH, Gateway, CAS \\
$\hat{L}_i$ & link-quality estimate & CH, join, CAS \\
$\hat{D}_{iB}$ & distance-to-BS feature & CH, Gateway, CAS \\
$\hat{C}_i$ & local centrality surrogate & CH, Gateway \\
$\hat{\rho}_i$ & neighborhood density & CH fraction, CAS \\
$\hat{U}_i$ & recent forwarding utilization & load control \\
$p_i$ & historical CH-duty penalty & CH fairness \\
\bottomrule
\end{tabular*}
\end{table}
...
CH election uses a normalized suitability score over residual energy, node centrality, neighborhood degree, distance to the BS, and link quality. A fairness penalty reduces repeated CH duty by multiplying the score by $1-\lambda p_i$, where $p_i$ records prior CH use and $\lambda$ is fixed for all environments. This makes CH election energy-aware without turning CH rotation into the dominant objective.

After CH election, non-CH nodes join one CH using a fixed weighted rule over distance, link quality, and residual energy. The exact weights and feasibility threshold are reported with the experimental configuration in Table~\ref{tbl:aeris_params}. If no feasible CH is available, the node attaches to the best available CH and the later uplink selector handles the weak path. This separation keeps local cluster association simple while leaving long-range reliability decisions to the Gateway/Skeleton layer.
...
\end{equation}
where $\bar{D}_i$ is normalized CH-to-BS distance, and $\hat{C}_i$, $\hat{L}_i$, and $\hat{E}_i$ are normalized centrality, link quality, and residual energy. The fixed publication weights are reported with the experimental configuration in Section~IV-A.
...
\begin{table}[t]
\caption{AERIS decision stages in one round.}
\label{tbl:aeris_round}
\centering
\scriptsize
\setlength{\tabcolsep}{2pt}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lll@{}}
\toprule
Stage & Main rule & Output \\
\midrule
Feature refresh & update energy/link/neighborhood & node state \\
CH election & fuzzy CH score with duty penalty & CH set \\
Association & distance/link/energy join score & clusters \\
CAS & direct/chain/two-hop fixed scores & local mode \\
Gateway & fixed CH relay score & uplink relay \\
Skeleton & sparse reserve eligibility & fallback path \\
Accounting & delivery and energy update & next-round state \\
\bottomrule
\end{tabular*}
\end{table}
...
\begin{table}[t]
\caption{Baseline roles in the evaluation harness.}
\label{tbl:baseline_roles}
\centering
\scriptsize
\setlength{\tabcolsep}{2pt}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lll@{}}
\toprule
Protocol & Routing rule & Role \\
\midrule
LEACH & randomized clustering & classical cluster baseline \\
PEGASIS & chain aggregation & lifetime-oriented baseline \\
HEED & residual-energy CH election & energy-aware cluster baseline \\
TEEN & threshold-triggered reporting & event-style baseline \\
CTP-style & link-cost forwarding & collection-tree boundary \\
RPL-MRHOF-style & accumulated path cost & RPL-style boundary \\
\bottomrule
\end{tabular*}
\end{table}
...
The statistical unit is an environment-scale-protocol cell. For the boundary sweep, each plotted bar is the sample mean AERIS-minus-best-baseline PDR gap over the 30-seed cell, and the whiskers in Fig.~\ref{fig:ns3_boundary} report an approximate 95\% confidence interval for that gap. The figure is intentionally not a significance heatmap. Its job is to show the deployment boundary and to identify where practical gaps are large enough to matter. For this reason, an absolute mean gap below 0.1 percentage point is treated as a near-tie even if the mean ordering names a winner.

The ablation uses the same seed structure and environment-scale grid, but compares full AERIS with one disabled component at a time. The reported contribution is full-minus-ablated PDR in percentage points. Significance counts in Fig.~\ref{fig:ablation} are computed per environment across the seven node scales after Holm correction. This convention is deliberately conservative: it avoids using the 3360-run count as if every packet or round were an independent statistical sample.

The five-protocol NS-3 comparison anchors the classical-baseline ranking; the seven-protocol sweep maps the boundary against simplified collection-style baselines; the ablation attributes the gain; and the collision-aware stress layer provides stress evidence rather than a second canonical ranking. Lifetime, FND, hops, and energy expose load concentration. In Fig.~\ref{fig:ablation}, each significance count summarizes Welch-type cell tests with Hedges' $g$ and Holm correction. In Fig.~\ref{fig:ns3_boundary}, winner labels alone should not be read as significance claims.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig2_boundary.pdf}
\caption{Expanded NS-3 boundary sweep across four environments and 50--1000 nodes ($n=30$ per cell). Each bar reports mean PDR(AERIS) minus the strongest non-AERIS baseline in percentage points; whiskers show approximate 95\% confidence intervals for the gap. Positive bars favor AERIS, negative bars favor the strongest boundary baseline, and the gray band marks the $\pm$0.1 percentage-point near-tie region. The title reports AERIS rank-1 coverage against classical/all baselines (C/A).}
\label{fig:ns3_boundary}
\end{figure}

Figure~\ref{fig:ns3_boundary} gives the main boundary result from the seven-protocol NS-3 sweep as signed PDR margins against the strongest non-AERIS baseline. This replaces a protocol-line plot because many high-PDR curves overlap visually near the top of the axis; the signed margin exposes the actual deployment boundary more directly. Against classical WSN protocols alone, AERIS is rank-1 in 21 of 28 environment-node cells and top-2 in all 28. This supports the narrower classical-baseline claim that AERIS is a reliability-first design that remains effective outside the benign office regime.
...
\begin{table}[t]
\caption{Separate fixed 100-node trade-off summary. The broader sweeps in Figs.~\ref{fig:ns3_boundary}--\ref{fig:ablation} cover 50--1000 nodes; this table pools the fixed 100-node block for energy, lifetime, FND, and hop reporting. Bold indicates AERIS. NR = no first-node death before the 300-round horizon.}
