...
Equations~(1)--(6) are intentionally ordered to mirror that control flow: state acquisition, CH election, member association, Gateway scoring, CAS mode selection, and first-hit uplink precedence. The paper is therefore not relying on a large number of equations for their own sake; instead, each equation names one routing decision that the implementation actually executes.
...
\begin{table}[t]
\caption{Separate fixed 100-node trade-off summary. The broader sweeps in Figs.~\ref{fig:classical_margin}--\ref{fig:ablation} cover 50--1000 nodes; this table pools the fixed 100-node block for energy, lifetime, FND, and hop reporting. Bold indicates AERIS. NR = no first-node death before the 300-round horizon.}
