...
The statistical unit is an environment-scale-protocol cell. For the boundary sweep, each plotted point is the sample mean over the 30-seed cell, and the AERIS band in Fig.~\ref{fig:ns3_boundary} reports an approximate 95\% mean interval. The figure is intentionally not a significance heatmap. Its job is to show the deployment boundary and to identify where practical gaps are large enough to matter. For this reason, an absolute mean gap below 0.1 percentage point is treated as a near-tie even if the mean ordering names a winner.
