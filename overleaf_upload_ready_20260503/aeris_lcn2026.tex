...
\footnotesize
\textbf{Fixed AERIS parameters used in all reported runs.}
\setlength{\tabcolsep}{2pt}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}p{0.28\columnwidth}p{0.66\columnwidth}@{}}
\toprule
Block & Fixed value / rule \\
\midrule
CH election & base ratio 0.08, LQI/density adjusted, clipped to $[0.05,0.20]$; fairness factor $1-0.25p_i$ \\
Cluster join & distance/link/energy weights $0.45/0.40/0.15$; minimum PRR 0.20 \\
Gateway & $w_D/w_C/w_L/w_E=-0.7/0.3/0/0$; one Gateway retry and direct rescue enabled \\
CAS stability & EMA 0.2; minimum confidence 0.2; uncertainty penalty disabled; rule override enabled \\
Skeleton & PCA-axis CH backbone, weights 0.7/0.3, far-CH quantile 0.75; reserve fallback only \\
\bottomrule
\end{tabular*}
\vspace{1pt}
\scriptsize
...
\toprule
CAS mode & $E$ & $L$ & $D_B$ & $R$ & $\rho$ & $F$ \\
\midrule
Direct & .35 & .65 & -.25 & -.05 & .10 & -.05 \\
Chain & .30 & .40 & .20 & .20 & .20 & -.05 \\
Two-hop & .20 & .25 & .50 & .15 & .05 & -.05 \\
\bottomrule
\end{tabular*}
\vspace{2pt}
\textbf{Algorithm 1: AERIS round.}
\begin{enumerate}
\setlength{\itemsep}{0pt}
\item Estimate normalized energy, LQI, neighborhood geometry, density, and queue/use summaries.
\item Select CHs with fuzzy score plus fairness penalty; associate each member to the best CH by distance, link quality, and CH energy.
\item For each cluster, choose direct, chain, or two-hop CAS forwarding from Eq.~(2) and rule overrides; aggregate at the CH.
\item Rank Gateway candidates using Eq.~(1); build the sparse Skeleton reserve over current CHs.
\item For each CH uplink, try direct, then Gateway, then Skeleton, then one final direct attempt; update energy, link statistics, alive state, and metrics.
\end{enumerate}
\end{minipage}

This specification is deliberately rule-based: the same state produces the same routing decision, and no coefficient is selected using environment-level test results. AERIS is therefore evaluated as an engineering mechanism rather than as a formal routing optimum.
