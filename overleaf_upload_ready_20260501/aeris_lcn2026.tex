...
\caption{AERIS round pipeline framework. Phase 1 builds clusters over an abstract topology by combining round-state sensing, CH scoring, and cluster association. Phase 2 uses CAS to select direct, chain, or two-hop intra-cluster forwarding, then aggregates packets at the CH. Phase 3 performs a pre-transmission uplink check: direct uplink is the baseline, Gateway-assisted uplink is the main gain path, Skeleton is a reserve path, and one-shot fallback is only used as the last branch.}
