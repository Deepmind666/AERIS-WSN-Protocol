...
\caption{AERIS round framework. Phase 1 combines an abstract topology inset with a compact legend that maps the role symbols M, R, CH, GW, and BS, together with intra-cluster, uplink, and skeleton links, and three process cards for round-state sensing, CH scoring, and cluster association. Phase 2 contains the CAS selector with direct, chain, and two-hop modes, the sparse intra-cluster forwarding sketch, and CH-level packet aggregation. Phase 3 places a pre-transmission decision spine beside four route outcomes: direct uplink, Gateway-assisted uplink as the main gain path, Skeleton reserve, and one-shot fallback.}
