...
We use four complementary settings. The five-protocol NS-3 audit compares AERIS against LEACH, PEGASIS, HEED, and TEEN across four environments and seven node scales from 50 to 1000 nodes ($n=30$ per cell). The expanded seven-protocol boundary sweep adds simplified CTP- and RPL-MRHOF-style collection baselines over the same grid; these are controlled collection-style approximations, not full LLN stacks. A 3360-run NS-3 ablation compares full AERIS with no-Gateway, no-CAS, and no-CH-score variants. A collision-aware Python stress layer adds explicit collision and relay contention; LEACH, HEED, and TEEN are minimally relay-enabled there for multi-hop comparability. A separate fixed 100-node publication block reports pooled PDR, consumed energy, lifetime, first-node death, and hop count, and a 400-replicate-per-cell mechanism study covers 12 cells and explains which AERIS mechanisms actually fire.
...
\subsection{Artifact Layout and Reproducibility}
The submission is split into a paper package and a data pack so that the reported figures can be rebuilt without searching through the manuscript tree. \texttt{fig2\_fig5\_data/} contains the source CSV and JSON files together with the scripts that regenerate the classical audit, boundary evidence, stress layer, ablation heatmap, mechanism study, and pooled trade-off summary. \texttt{00\_final\_outputs/} stores the final rendered PDFs and PNGs, while \texttt{LCN26\_AERIS\_overleaf/} stores the camera-ready manuscript and the figure copies used by \LaTeX. The packaged archive \texttt{LCN26\_AERIS\_overleaf.zip} is the exact handoff unit for Overleaf upload.

The workflow figure is the only manually edited asset; every other figure is rebuilt from a checked-in source file and a fixed script. This makes the canonical audit, expanded boundary note, stress evidence, ablation matrix, and mechanism study reproducible from one repository state, with the same seed blocks and environment-scale grid used for the reported results. The split also keeps the repository audit-friendly: a reviewer can inspect raw inputs, derived summaries, and final plots separately, instead of treating the PDF as the only artifact.

For the boundary comparison, the manuscript intentionally keeps the simplified CTP/RPL-MRHOF variants separate from canonical ranking evidence. Their role is to bound the claim, not to replace full LLN stacks. The same separation is used throughout the paper for canonical ranking, boundary evidence, stress evidence, and mechanism diagnosis.
