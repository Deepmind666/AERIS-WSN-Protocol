...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File data\figure_reproduction\rebuild_figures_from_pack.ps1
```

The script regenerates Fig. 2--Fig. 5, mirrors the PDFs into
`paper/LCN26_AERIS_overleaf/figures/`, and updates `00_final_outputs/`.

Individual commands:

```powershell
python data\figure_reproduction\scripts\build_lcn26_ns3_canonical_margin.py
python data\figure_reproduction\scripts\build_lcn26_strict_compact.py
python data\figure_reproduction\scripts\build_lcn26_ns3_ablation_figure.py
python data\figure_reproduction\scripts\build_lcn26_compact_tail_figures.py
```
...
`paper/LCN26_AERIS_overleaf/figures/fig1_workflow.png`; it is not a numeric experiment
figure.
...
```powershell
Get-ChildItem -Path data\figure_reproduction -Recurse -File |
  Where-Object { $_.Name -ne 'MANIFEST.sha256' } |
  Sort-Object FullName |
  Get-FileHash -Algorithm SHA256 |
  ForEach-Object { "$($_.Hash)  $(Resolve-Path -Relative $_.Path)" } |
  Set-Content -Encoding ASCII data\figure_reproduction\MANIFEST.sha256
```
