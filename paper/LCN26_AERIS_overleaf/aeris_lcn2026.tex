...
We use four complementary settings: a five-protocol NS-3 audit, an expanded seven-protocol boundary sweep, a 3360-run NS-3 ablation, and a collision-aware Python stress layer. A fixed 100-node block and a 400-replicate mechanism study report the trade-off and module activity.
...
The five-protocol NS-3 comparison anchors the classical-baseline ranking; the expanded seven-protocol sweep defines the boundary; the ablation attributes the gain; and the collision-aware stress layer is stress evidence, not a second canonical ranking. Lifetime, FND, hops, and energy expose load concentration. For Fig.~\ref{fig:ablation}, module effects are checked with Welch-type cell tests, Hedges' $g$, and Holm correction.
