...
\abstract{This paper presents AERIS, an environment-aware hierarchical routing protocol for wireless sensor networks, evaluated with \texttt{pdr\_expected}. In a four-environment 100-node matrix (30 seeds per environment), AERIS achieves the highest mean PDR among LEACH, PEGASIS, HEED, and TEEN. In a stricter large-scale matrix (100--1000 nodes) with balanced sampling (3200 replicates per environment-node-protocol cell) and explicit MAC-collision and multi-hop relay modeling, AERIS remains rank-1 in three environments, while PEGASIS dominates indoor\_office at high scale. Matched tx-power tests (5 vs 15 dBm, n=600) show strong environment-dependent responses. Matched patch-control tests (n=1000 vs 1000) show consistent absolute PDR degradation under strict physics settings, indicating deployment-time calibration requirements. NS-3 is used as cross-platform trend validation for AERIS versus LEACH. The results position AERIS as a reliability-oriented option for harsh channels with explicit environment-scoped operating boundaries.}
...
\item publication-tier evidence matrices that separate core reliability reporting (100-node + v50-rigor scalability matrix) from stress/sensitivity calibration blocks (S9/S10/S11);
...
\end{equation}
which is non-decreasing with each component probability. AERIS does not assume perfect independence in practice, but this expression explains why adding controlled forwarding alternatives can improve reliability in harsh channels. CAS then chooses the lower-cost intra-cluster collection mode before this uplink stage, so routing overhead is bounded while uplink diversity is preserved.
...
Figure~\ref{fig:scalability_panel} together with Table~\ref{tab:robust_snapshot} provides the v50-rigor trend-and-significance view.
...
At the protocol level, Holm-significant cell counts in S11 are 24/24 for AERIS, 24/24 for LEACH, 0/24 for PEGASIS, 24/24 for HEED, and 24/24 for TEEN. Together with Figure~\ref{fig:s11_patch_control}, this shows that the strict patch strongly affects most protocol families, while PEGASIS remains nearly invariant in this block.
...
\item The v50-rigor large-scale matrix (n=3200 per cell) includes explicit collision and relay constraints, but does not replace hardware-in-the-loop validation.
\item S10 and S11 show that stricter physics assumptions and tx-power changes can materially alter absolute PDR, so deployment requires environment-specific calibration.
