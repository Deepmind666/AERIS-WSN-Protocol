...
Wireless sensor network (WSN) routing is classically optimized for energy reduction, but practical deployments also require stable delivery under heterogeneous channels \cite{Akyildiz2002Survey,Rault2016Energy,Kandris2020}. This creates a multi-objective tension among reliability, hop-based latency, and energy use.

Existing protocols represent distinct operating points: LEACH emphasizes clustered forwarding \cite{Heinzelman2000LEACH}, PEGASIS emphasizes chain-based aggregation \cite{Lindsey2002PEGASIS}, HEED emphasizes residual-energy-aware head selection \cite{Younis2004HEED}, and TEEN emphasizes event-triggered reporting \cite{Manjeshwar2001TEEN}. Prior channel studies also show that low-power links can be asymmetric and environment-sensitive under realistic propagation conditions \cite{Zuniga2004,Zuniga2007Asymmetry,Baccour2012RLQE}.

AERIS prioritizes reliability under realistic channel variability as a lightweight hierarchical protocol. The design objective is not universal dominance; rather, it is robust performance across multiple channel environments while preserving deployability on resource-constrained nodes.
...
Recent adaptive or learning-based methods improve specific settings but often require heavier runtime or training overhead \cite{Ren2024,Okine2024,Chen2019Survey,Naeem2024DRL,Scientific2025DRLGNN,Nature2024NNILEACH,AI2025Survey,Manogaran2025DDQN,Tan2026MOGALSA}. Recent Sensors/IoT studies report gains from context-aware or hybrid routing under constrained networks \cite{ElFouly2023ILP,Dan2024EMRAR,Bhukya2025Hybrid,Kaddi2024GWOCSMA,Wang2024Geographic,Tariq2024A,Hassanien2025DLHEED,Ti2026HEOCP,Juwaied2025DEEC}.

Additional recent studies show that routing quality is strongly coupled to link asymmetry, deployment context, and adaptive control-policy design \cite{Jain2025Optimized,Ogundile2025PPRP,Singh2025Enhanced,Tossa2025Deployment,Prince2025Hotspot,Qin2026WBAN}. NLOS-aware estimation methods further demonstrate that environment uncertainty degrades localization and, by extension, distance-dependent routing decisions \cite{Qin2024NLOSError}. These results motivate explicit environmental adaptation in protocol design while keeping inference and control logic lightweight.

Hybrid cluster-chain switching and gateway-assisted hierarchical forwarding are not new concepts per se; earlier work explored LEACH-PEGASIS hybrids and energy-efficient hierarchical gateway routing. AERIS differs in two specific respects: (1) CAS is not a learned policy or meta-heuristic but a fixed-coefficient linear scoring rule whose mode weights are shared across all environments, making behavior fully auditable and reproducible without training; (2) the Gateway module operates as a conditional fallback layer (activated only when direct uplink quality is insufficient), not as a permanent relay tier, which avoids single-point-of-failure concentration at gateway nodes.

In this manuscript, the focus is a rule-based protocol with explicit reproducibility constraints and auditable experiment metadata.
...
\end{equation}
where $N_{\mathrm{delivered}}$ is the number of unique data packets received at the base station and $N_{\mathrm{expected}}$ is the number of packets that \emph{alive} source nodes were scheduled to generate during all rounds (i.e., one packet per alive node per round). This denominator is protocol-independent: it counts expected application-layer transmissions regardless of whether a protocol suppresses sending (as in TEEN's threshold-based reporting). All publication conclusions in this draft are based on this metric.
...
\begin{table}[H]
\caption{Fixed initialization coefficients for publication-tier runs (shared across all four environments).}
...
\end{tabular}
\end{table}
Runtime stage-adaptive updates are bounded, deterministic under fixed seeds, and logged as reproducibility metadata. Source-to-manuscript traceability is provided in the supplementary coefficient mapping material.
...
\begin{itemize}
\item \textbf{Reliability under harsh channels}: when direct CH-to-BS links become unstable, Gateway support and Skeleton routing create additional forwarding opportunities instead of relying on a single uplink path. If the elected gateway node itself fails (energy depletion or link loss), the cascaded fallback logic in Eq.~(4) bypasses the gateway layer and attempts Skeleton routing or best-effort direct transmission; gateway re-election occurs at the next round boundary without protocol-level oscillation.
...
\subsection{Round Execution and Complexity}
Each round executes the following fixed sequence (held constant across protocols to preserve comparability):
\begin{enumerate}
...
\item Forward CH traffic to BS and log metrics.
\end{enumerate}
Figure~\ref{fig:aeris_workflow} summarizes this execution and evidence pipeline.

Let $N$ be the node count and $H$ be the CH count in one round. CH election and clustering are $O(NH)$, CAS selection is $O(1)$ per cluster, and Gateway ranking is $O(H\log H)$. Skeleton update scales with active candidate links $E_a$. The dominant per-round cost is therefore
\begin{equation}
T_{\mathrm{round}} = O(NH + H\log H + E_a),
\end{equation}
where $H \ll N$ in the tested regimes. This expression is an implementation-level bound, not a formal optimality claim.

\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{fig0_aeris_workflow_20260302_s97.pdf}
...
\noindent\textbf{Channel model.}
Each environment uses a log-distance path loss model with log-normal shadowing.
Received power at distance $d$ is
$P_{\mathrm{rx}}(d) = P_{\mathrm{tx}} - \mathrm{PL}(d_0) - 10\,n\log_{10}(d/d_0) - X_\sigma$,
where $n$ is the path loss exponent and $X_\sigma \sim \mathcal{N}(0,\sigma^2)$ is shadowing.
Table~\ref{tab:channel_params} lists the per-environment parameters.

\begin{table}[H]
\caption{Channel model parameters per environment.}
...
\midrule
indoor\_office    & 2.0 & 4.5 \\
indoor\_factory   & 2.7 & 8.5 \\
outdoor\_urban    & 3.4 & 12.0 \\
outdoor\_suburban & 2.8 & 7.5 \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Energy model.}
All protocols share the first-order radio model:
$E_{\mathrm{tx}}(k,d)=E_{\mathrm{elec}}\cdot k + \epsilon \cdot k \cdot d^{\alpha}$,
$E_{\mathrm{rx}}(k)=E_{\mathrm{elec}}\cdot k$,
with $E_{\mathrm{elec}}=50$\,nJ/bit, $\epsilon_{\mathrm{fs}}=10$\,pJ/bit/m$^2$ (free space, $d<d_0$),
$\epsilon_{\mathrm{mp}}=0.0013$\,pJ/bit/m$^4$ (multipath, $d\geq d_0$),
crossover distance $d_0=\sqrt{\epsilon_{\mathrm{fs}}/\epsilon_{\mathrm{mp}}}\approx87$\,m, packet size $k=4000$\,bits, and initial energy $E_0=0.5$\,J per node.

\noindent\textbf{MAC and collision model.}
In the primary large-scale matrix (strict-physics mode), a slot-level collision flag is enabled: if two or more nodes in the same cluster transmit in the same TDMA slot, all colliding packets are dropped.
Baseline protocols that lack native multi-hop relay (LEACH, HEED, TEEN) are augmented with a simple greedy relay mechanism to ensure fair comparison on multi-hop paths.
No capture effect or retransmission is modeled; a collided packet is unconditionally lost.
...
\item no reliability override (forced reliable delivery disabled).
\end{itemize}
...
This study reports two regime tiers for scalability interpretation and keeps the naming consistent across all tables and captions. The primary tier is the large-scale matrix (explicit collision-and-relay settings, balanced n=3200 per cell) and is used for large-scale ranking claims. The secondary tier (stress and sensitivity blocks) is retained for matched stress comparisons (patch minus control) and tx-power diagnostics, not for replacing the primary ranking matrix.

For readability, we refer to patch-minus-control contrasts as \emph{stress-delta} comparisons (i.e., the signed difference in PDR between the patch arm with strict physics enabled and the control arm with legacy configuration, under matched environment and node settings) throughout Sections 5.4 and 5.6.
...
Stress-delta matrix A (patch minus control) & 4 env $\times$ 6 node counts $\times$ 5 protocols & patch n=1000, control n=600 & Physics-stress calibration \\
...
Stress-delta matrix B (matched patch-minus-control) & 4 env $\times$ 6 node counts $\times$ 5 protocols & patch n=1000, control n=1000 & Matched strict-physics confirmation \\
...
\end{equation}
because variance equality is not assumed across protocols and environments. Family-wise error control is handled by Holm correction within each reported comparison family. Each family is defined as the set of pairwise comparisons within a single table or figure: for example, the 100-node matrix (Table~\ref{tab:pdr100}) defines one family of $m=4$ tests (AERIS vs each baseline per environment); the scalability heatmap (Figure~\ref{fig:s8_significance_map}) defines one family per environment column ($m=4$ baselines $\times$ 6 node counts $= 24$); the ablation analysis (Table~\ref{tab:ablation_gateway}, Figure~\ref{fig:ablation_panel}) defines one family of $m=8$ tests (two ablation variants $\times$ four environments). Effect size is reported with Hedges' $g$:
\begin{equation}
g = J\cdot\frac{\bar{x}_1-\bar{x}_2}{s_p},\quad
J=1-\frac{3}{4(n_1+n_2)-9},
\end{equation}
and interpreted as small (0.2), medium (0.5), and large (0.8) in absolute value following Cohen's heuristics; however, because these thresholds are not calibrated for simulation-derived PDR distributions, we primarily use $g$ to convey practical magnitude and always report the accompanying absolute $\Delta$PDR alongside it. Because Welch's $t$ does not assume equal variance but Hedges' $g$ uses pooled $s_p$, we verified that within-cell coefficient-of-variation ratios remain below 2:1 in publication-tier matrices; when this condition is violated the pooled standardizer would overstate or understate the effect, and we flag such cells as unreliable in the significance maps. Confidence intervals are reported as two-sided 95\% intervals. All significance statements in this manuscript are based on Holm-corrected $p$ values. For large-$n$ cells, interpretation is based jointly on corrected $p$ values, effect size, and absolute PDR delta, because with $n=3200$ even trivially small effects reach statistical significance. Reported table-wise standard deviations are treated as descriptive values from the corresponding result files; inferential conclusions are computed from raw replicate-level records.
...
\begin{table}[H]
\caption{Legacy comparability matrix (strict collision/relay flags disabled): PDR by environment at 100 nodes (n=30 per environment-protocol cell).}
\label{tab:pdr100}
\centering
\resizebox{\linewidth}{!}{
...
Because Table~\ref{tab:pdr100} is generated from the legacy 100-node regime (without strict collision/relay flags), its absolute values are intentionally not pooled with the primary large-scale matrix. Figure~\ref{fig:env_panel} visualizes the cross-environment PDR ordering at this scale.

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig1_env_pdr_panel_20260302_s97.pdf}
\caption{PDR comparison at 100 nodes (4 environments, n=30). Shared 0--1 y-axis across panels; outdoor\_urban inset highlights the low-range baseline band (0--0.16).}
\label{fig:env_panel}
\end{figure}
...
Gateway is positive in three environments and near-neutral in indoor\_office (Table~\ref{tab:ablation_gateway}, Figure~\ref{fig:ablation_panel}). CAS is not consistently positive across environments.
In indoor\_office, the Full-vs-no\_gateway gap is +0.0002 (absolute PDR), so this cell is treated as practically neutral. After Holm--Bonferroni correction ($m=8$), only two comparisons remain significant: Gateway in indoor\_factory ($\Delta=-2.25$~pp, $p_{\mathrm{Holm}}=0.005$) and CAS in outdoor\_urban ($\Delta=+2.44$~pp, $p_{\mathrm{Holm}}=0.015$); see Panel~(c) of Figure~\ref{fig:ablation_panel}.

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig2_ablation_panel_20260304_s98.pdf}
\caption{Ablation analysis (n=30 per cell, four environments): (a)~PDR heatmap with per-cell delta relative to Full; (b)~marginal effects of removing Gateway or CAS with 95\% Welch confidence intervals (filled markers denote Holm-significant comparisons at $\alpha=0.05$); (c)~Holm-corrected significance overview with effect direction.}
\label{fig:ablation_panel}
\end{figure}

\subsection{Scalability (100--1000 Nodes; Balanced n=3200 per Cell)}
\begin{table}[H]
\caption{PDR ranking at 1000 nodes from the primary large-scale matrix (n=3200 per cell). Values are means; per-cell SDs range from 0.001 to 0.05 and are reported in the significance heatmap (Figure~\ref{fig:s8_significance_map}).}
\label{tab:scale1000}
\centering
...
AERIS ranks first in 18 of 24 environment-scale cells; the remaining 6 cells (all indoor\_office) are won by PEGASIS (Table~\ref{tab:scale1000}).
...
\begin{table}[H]
\caption{Holm-corrected significance snapshot at 1000 nodes (AERIS vs strongest baseline in each environment; n=3200 per cell).}
...
Figure~\ref{fig:scalability_panel} shows the corresponding scalability trends across all five protocols.

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig3_scalability_panel_20260302_s97.pdf}
\caption{Primary large-scale scalability trends (n=3200 per cell, 95\% CI bands). Each panel shows all five protocols over six node counts on a shared 0--1 y-axis.}
\label{fig:scalability_panel}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig8_s8_significance_heatmap_20260302_s97.pdf}
\caption{Primary-matrix strongest-baseline map. (a) Cell-wise \(\Delta\)PDR to the strongest baseline; tags L/P/H/T denote LEACH/PEGASIS/HEED/TEEN. (b) Effect-size magnitude \(|g|\) with Holm significance; hatched cells are non-significant.}
\label{fig:s8_significance_map}
\end{figure}

Figure~\ref{fig:s8_significance_map} encodes three quantities per cell: the baseline tag (L/P/H/T), signed \(\Delta\)PDR, and Holm significance with \(|g|\). This makes delta and effect-size statements in Table~\ref{tab:robust_snapshot} fully traceable to a concrete baseline protocol.

\subsection{Rigor-Patch Pilot Sanity Check (n=60)}
To test simulator-physics consistency before the full rerun, we ran a local pilot with collision and baseline multi-hop relay enabled, using the same environment taxonomy and nodes \{100, 500, 1000\} with n=60 per cell. This pilot is retained as a diagnostic snapshot (Table~\ref{tab:rigor_patch} in Appendix~A) and is not used in primary claim tables.

The key pilot observation is monotonicity recovery: AERIS PDR decreases with scale in all four environments. In indoor\_office, PEGASIS is higher than AERIS under this stricter setting, while AERIS remains higher in the other three environments. The pilot therefore served as a gate check for launching the full primary large-scale matrix and is reported only for diagnostic completeness.

\subsection{Stress-Delta Matrix A (Four Environments)}
To reduce seed and sampling confounders, we ran Stress-delta matrix A (patch-minus-control comparison) in all four environments (indoor\_office, indoor\_factory, outdoor\_urban, and outdoor\_suburban). The patch mode enables collision and baseline relay mechanisms; the control mode uses the legacy control configuration. Patch cells use n=1000 and control cells use n=600 (Table~\ref{tab:s9_patch_control} in Appendix~A).

Three observations are robust in this stress-delta matrix. First, AERIS remains above LEACH/HEED/TEEN under patch mode in all four environments at 1000 nodes. Second, PEGASIS is minimally affected by the patch, with near-zero deltas and non-significant tests in all 24 PEGASIS cells\footnote{Across the 24 PEGASIS cells in this stress-delta matrix, \(|\Delta_{\text{stress-delta}}| \le 0.0044\), all Holm-adjusted \(p\)-values are \(1.0000\), and \(\max |g| = 0.0903\), consistent with a near-zero effect.}. A technical hypothesis is that serialized chain forwarding in PEGASIS exposes fewer branch decisions to stress toggles than cluster-centric protocols, so fewer control points are perturbed under the current implementation path. Third, AERIS patch-minus-control deltas are significant in all 24 AERIS cells after Holm correction, with the largest absolute drop in outdoor\_urban at high scale. Table~\ref{tab:s9_pegasis_1000} (Appendix~A) provides the 1000-node PEGASIS snapshot supporting this interpretation.

In matched stress-delta matrix B, PEGASIS shows exact zero deltas in indoor\_factory (all six node counts). We treat this as an implementation-coupling anomaly requiring dedicated implementation audit, not as physical invariance evidence. The patch can also be overly aggressive for baseline paths at high scale, so this block is interpreted as stress-delta evidence rather than as a final replacement matrix. To remove sample-size asymmetry from stress-delta matrix A, we run matched stress-delta matrix B (patch \(n=1000\), control \(n=1000\) per cell) and report it separately below.

Consistency between stress-delta matrix A and matched stress-delta matrix B is assessed from the matched-delta tables and significance files; both matrices preserve the same direction pattern for AERIS across environment-scale cells.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig6_s10_delta_maps_20260302_s97.pdf}
\caption{Power-sensitivity delta maps in absolute PDR units (rows: tx5--tx10, tx10--tx15, tx5--tx15; columns: four environments; five protocols, six node counts). Cross markers denote Holm non-significant cells.}
\label{fig:s10_power_sensitivity}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig10_s10_absolute_profiles_20260302_s97.pdf}
...
\subsection{Matched Stress-Delta Confirmation Matrix (Four Environments, n=1000 vs 1000)}
To remove remaining sample-size asymmetry from stress-delta matrix A, we completed matched stress-delta matrix B in which both patch and control arms use n=1000 per environment-node-protocol cell (4 environments $\times$ 6 node counts $\times$ 5 protocols). This matrix directly tests whether stricter physics assumptions improve or degrade reliability under matched sampling (Table~\ref{tab:s11_aeris_delta} in Appendix~A).

In the matched matrix, AERIS patch-minus-control deltas are negative and significant in all 24 environment-scale cells after Holm correction (24/24 significant; Hedges' $g$ range: -0.47 to -15.36). Therefore, under the current patch configuration (MAC collision and multi-hop relay enabled), stricter physics assumptions reduce reliability rather than increase it. This matrix is interpreted as realism-stress evidence, not as a replacement for the primary ranking matrix.

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig5_s11_patch_control_delta_20260302_s97.pdf}
\caption{Matched stress-delta evidence. (a) AERIS delta trajectories over node counts (patch - control). (b) Protocol deltas at 1000 nodes across environments. Negative deltas indicate lower PDR under patch mode.}
\label{fig:s11_patch_control}
\end{figure}

At the protocol level, Holm-significant cell counts in matched stress-delta matrix B are 24/24 for AERIS, 24/24 for LEACH, 0/24 for PEGASIS, 24/24 for HEED, and 24/24 for TEEN. Together with Figure~\ref{fig:s11_patch_control}, this confirms that the strict patch strongly affects most protocol families, while PEGASIS remains nearly invariant in this matched-delta block. A working technical hypothesis is that the current patch primarily perturbs cluster-based decision points (collision-sensitive parallel uplinks and relay augmentation), whereas PEGASIS uses a serialized chain-forwarding pattern with fewer concurrent uplink competitions in this simulator path; this hypothesis remains subject to explicit code-path instrumentation.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig4_tradeoff_panel_20260302_s97.pdf}
...
\end{tabular}
\end{table}
Table~\ref{tab:energy_snapshot} shows that AERIS consumes the least total energy among the five protocols, yet exhibits the shortest network lifetime (215 rounds vs 300 for PEGASIS/LEACH/HEED). The mechanism is relay-load concentration: Gateway and Skeleton forwarding paths improve delivery but funnel traffic through structurally important nodes, causing earlier first-node death than chain-rotation protocols that distribute load more uniformly. For deployment, this means reliability-first operation under AERIS should be paired with environment-tuned duty cycling or node redundancy to mitigate relay bottleneck depletion.

\subsection{NS-3 Trend Validation (AERIS vs LEACH, n=30)}
NS-3 validation is used as a cross-platform trend check rather than a numerical-equivalence proof. Formal significance claims in this block remain restricted to AERIS-versus-LEACH. Under aligned multi-environment settings, AERIS is directionally not lower than LEACH in 27 of 28 tested environment-scale cells; the single exception is indoor\_office at 200 nodes (AERIS--LEACH diff $=-0.0004$, not significant). At 100 nodes, significance is confirmed in 3/4 environments (indoor\_factory, outdoor\_urban, outdoor\_suburban), while indoor\_office remains non-significant. In the 300/500-node extension, all eight environment-scale comparisons are significant after Holm correction; with the 800/1000 extension included, non-significance remains confined to indoor\_office at 1000 nodes. Table~\ref{tab:ns3_trend} reports the 100-node baseline, and Figure~\ref{fig:ns3_trend_panel} overlays PEGASIS/HEED/TEEN curves for descriptive context only.

\begin{table}[H]
\caption{NS-3 trend-level validation at 100 nodes (AERIS vs LEACH, n=30; metric: mean PDR). Per-cell SDs range from 0.003 to 0.04; full distributions are available in the data repository.}
\label{tab:ns3_trend}
\centering
\begin{tabular}{lccccc}
\toprule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig7_ns3_trend_panel_20260302_s97.pdf}
...
In this NS-3 block, inferential interpretation is restricted to the AERIS-versus-LEACH pair; context overlays are descriptive only.
This block supports trend consistency only; cross-platform numerical equivalence is not claimed. Across 7 node counts and 4 environments, 25/28 AERIS-versus-LEACH comparisons are significant; the non-significant cells are indoor\_office at node counts 100, 200, and 1000.
...
The manuscript separates three evidence blocks---100-node comparison, primary large-scale matrix (n=3200 per cell), and NS-3 trend validation---without cross-block numerical pooling, because each block answers a different question. Under stricter stress-delta settings, high-scale PDR decreases for most protocols; under tx-power sensitivity, power effects are strong but not globally monotonic. Practical interpretation therefore remains environment-scoped.

An additional boundary is causal interpretation across regime tiers. The difference between the legacy 100-node matrix and the primary large-scale matrix is not interpreted as a single-factor treatment effect, because two changes are introduced together in the primary rerun: (i) strict collision-and-relay physics is enabled, and (ii) previously identified baseline suppressors are removed. For this reason, cross-tier comparisons are used only for scope positioning, while matched stress-delta blocks are used for factor-specific direction checks.

Compared with recent adaptive-routing studies \cite{Ren2024,Okine2024,Naeem2024DRL,Scientific2025DRLGNN,Kaddi2024GWOCSMA,Wang2024Geographic,Tariq2024A,Jain2025Optimized,Ti2026HEOCP,Manogaran2025DDQN}, the main distinction is methodological: rather than reporting a single favorable matrix, this manuscript reports primary ranking evidence alongside matched stress/sensitivity blocks that expose when absolute reliability degrades. A secondary distinction is architectural: AERIS uses fixed-coefficient linear scoring (no training loop, no gradient updates, no policy network), so its inference cost is $O(K)$ per cluster per round, where $K$ is the number of candidate modes. This makes AERIS computationally lightweight and deployment-oriented, avoiding the memory, convergence, and non-stationarity risks associated with online learning. Direct numerical comparison with RL-based protocols is deferred to future work because fair comparison requires aligned reward definitions and convergence criteria that are not standardized across the cited studies.
...
Enabling the current patch configuration (MAC-collision plus multihop relay) reduces absolute AERIS PDR in all 24 matched cells, while relative ranking is preserved. This does not contradict the primary ranking result: ranking asks whether AERIS remains higher than alternatives under the same strict setting, while the matched stress-delta matrix asks how far the strict patch shifts each protocol relative to its own control path. Strict-physics penalties affect all protocols but non-uniformly across protocol families. The correct interpretation is therefore ``AERIS keeps relative advantages in harsh regimes while absolute reliability drops under stricter assumptions.''
...
\item Tx-power settings should be environment-tuned; monotonic power assumptions are not supported by the sensitivity block (see Section 5.5 for specific counter-examples).
\end{itemize}
...
Three boundaries are explicit in this draft (see Table~\ref{tab:deployment_summary} for environment-scoped guidance). First, the reported comparisons are simulator-based under the tested channel taxonomy. Second, NS-3 evidence is used for cross-platform trend validation (AERIS vs LEACH), not full five-protocol numerical alignment. Third, module contributions are context dependent: Gateway and CAS effects vary by environment and are not globally monotonic.
...
\item The primary large-scale matrix (n=3200 per cell) includes explicit collision and relay constraints, but does not replace hardware-in-the-loop validation. A priority next step is deploying AERIS on a TelosB or nRF52-based testbed (10--50 nodes) to confirm that the simulated environment-dependent PDR ordering holds under real radio impairments including multipath fading, clock drift, and co-channel interference from non-WSN devices.
\item Stress-delta matrix A (patch minus control), the tx-power sensitivity matrix, and the matched stress-delta matrix B show that stricter physics assumptions and tx-power changes can materially alter absolute PDR, so deployment requires environment-specific calibration.
\item In the matched stress-delta matrix, PEGASIS shows near-invariant patch-minus-control deltas; this behavior is treated as a simulator-coupling artifact requiring dedicated implementation audit. A working technical hypothesis is that serialized chain forwarding exposes fewer branch points to patch toggles than cluster-centric baselines under the current code path. In the primary large-scale ranking matrix, all protocols (including PEGASIS) are executed under the same strict collision-and-relay path without mixing a control arm, so this artifact affects interpretation of stress-delta blocks but does not alter standalone primary-matrix ranking values, which are computed under one strict path without control-arm subtraction.
...
\item In high-CV cells (e.g., outdoor\_urban at 800 nodes, where CV exceeds 0.33 across all three tx-power levels), mean$\pm$std may understate distributional complexity; median-based or quantile summaries are recommended for deployment-critical decisions in such near-threshold regimes.
\end{itemize}

\section{Conclusion}
AERIS is a reliability-oriented routing option for heterogeneous WSN channels. In the legacy 100-node comparability matrix, it achieves the highest mean PDR within the tested baseline set across all four environments. In the primary large-scale matrix (100--1000 nodes, four environments), AERIS is rank-1 in three environments (indoor\_factory, outdoor\_urban, outdoor\_suburban) and rank-2 in indoor\_office where PEGASIS remains dominant.

NS-3 provides trend-level cross-platform support for AERIS versus LEACH. The completed stress and sensitivity blocks show that stricter physics assumptions and tx-power changes materially affect high-scale behavior. In the matched stress-delta matrix, AERIS patch-minus-control deltas are negative and significant in all 24 environment-scale cells under the current patch setup.

The practical conclusion is environment-scoped deployment: AERIS remains strong for reliability-first operation in harsh channels, while benign indoor conditions at large scale may favor chain-forwarding baselines. Final parameterization should therefore be environment-tuned using matched stress and sensitivity evidence.
...
\begin{appendix}
\section{Diagnostic and Stress-Delta Tables}

The following tables are referenced in the main text for regime-transition diagnostics and stress calibration. They are not used for primary ranking claims.
...
\begin{table}[H]
\caption{AERIS PDR in stress-delta matrix A (patch-minus-control comparison, four environments).}
...
\begin{table}[H]
\caption{PEGASIS stress-delta snapshot at 1000 nodes from stress-delta matrix A (n=1000 patch, n=600 control).}
\label{tab:s9_pegasis_1000}
\centering
\begin{tabular}{lcccc}
\toprule
Environment & Delta & Hedges' $g$ & Holm $p$ & Significant \\
\midrule
...
\begin{table}[H]
\caption{AERIS patch-minus-control deltas in the matched stress-delta matrix (delta = patch - control).}
