...
This study reports two regime tiers for scalability interpretation and keeps the naming consistent across all tables and captions. The primary tier is the large-scale matrix (explicit collision-and-relay settings, balanced n=3200 per cell) and is used for large-scale ranking claims. The secondary tier (stress and sensitivity blocks) is retained for matched stress comparisons (patch minus control) and tx-power diagnostics, not for replacing the primary ranking matrix. For readability, we refer to patch-minus-control contrasts as \emph{stress-delta} comparisons (i.e., the signed difference in PDR between the patch arm with strict physics enabled and the control arm with legacy configuration, under matched environment and node settings) throughout Sections 5.4 and 5.6.
...
\caption{Primary large-scale scalability trends (n=3200 per cell, 95\% CI bands). Each panel includes all five protocols over six node counts on a shared 0--1 y-axis. Fine-grained baseline separation is provided in Figure~\ref{fig:s10_absolute_profiles} to keep this primary scalability panel structurally uniform.}
...
\caption{Primary-matrix strongest-baseline map. (a) Cell-wise \(\Delta\)PDR to the strongest baseline (in percentage points), where the strongest baseline is selected by the largest baseline mean in each environment-node cell; tags L/P/H/T denote LEACH/PEGASIS/HEED/TEEN. (b) Effect-size magnitude \(|g|\) to the strongest baseline with Holm significance marks and baseline tags. Hatched cells are Holm non-significant.}
\label{fig:s8_significance_map}
\end{figure}

Reading instruction for Figure~\ref{fig:s8_significance_map}: first identify the baseline tag (L/P/H/T) in each cell, then read signed \(\Delta\)PDR, and finally check Holm significance and \(|g|\); all three quantities refer to the same tagged baseline in that cell.
Figure~\ref{fig:s8_significance_map} makes the strongest-baseline identity explicit in every cell, so the delta and effect-size statements in Table~\ref{tab:robust_snapshot} remain fully traceable to a concrete baseline protocol rather than an implicit pooled reference.
...
Table~\ref{tab:s9_patch_control} provides the stress-delta baseline (patch minus control) used for cross-regime interpretation.
...
\caption{Power-sensitivity delta maps in absolute PDR units across all pairwise transmit-power comparisons (rows: tx5--tx10, tx10--tx15, tx5--tx15), four environments (columns), five protocols, and six node counts. No cells are filtered. Cross markers denote Holm non-significant cells.}
\label{fig:s10_power_sensitivity}
\end{figure}

Reading instruction for Figure~\ref{fig:s10_power_sensitivity}: read row by row (tx5--tx10, tx10--tx15, tx5--tx15), then compare protocols within each environment column; signs and magnitudes are absolute PDR deltas (not percentages), and cross-marked cells are Holm non-significant.
\begin{figure}[H]
\centering
...
Reading instruction for Figure~\ref{fig:ns3_trend_panel}: inferential interpretation is restricted to the primary pair (AERIS vs LEACH); context overlays (PEGASIS/HEED/TEEN) are descriptive only and are not used for Holm significance statements in this NS-3 block.
...
Matched stress-delta matrix B shows that enabling the current patch configuration (MAC-collision plus multihop relay) reduces absolute AERIS PDR in all 24 matched cells. This does not contradict the primary ranking result, because ranking and absolute-level behavior answer different questions: (i) ranking asks whether AERIS remains higher than alternatives under the same strict setting, while (ii) the matched stress-delta matrix asks how far the strict patch shifts each protocol relative to its own control path under identical sample sizes. In our data, strict-physics penalties affect all protocols, but the penalty profile is not uniform across protocol families. Therefore, the correct interpretation is not ``AERIS improves under patch'', but ``AERIS keeps relative advantages in harsh regimes while absolute reliability drops under stricter channel/relay assumptions.'' This distinction is central to deployment calibration.
