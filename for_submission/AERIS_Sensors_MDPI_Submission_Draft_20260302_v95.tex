...
\caption{PDR comparison panel at 100 nodes (4 environments, n=30). All panels use a single shared 0--1 y-axis for direct cross-environment comparability; the outdoor\_urban panel highlights the low-range baseline band (0--0.16) directly in-panel without a second inset chart.}
\label{fig:env_panel}
\end{figure}
...
\caption{Primary-matrix strongest-baseline map. (a) Cell-wise \(\Delta\)PDR (percentage points) to the strongest baseline per environment-node cell; tags L/P/H/T denote LEACH/PEGASIS/HEED/TEEN. (b) Effect-size magnitude \(|g|\) with Holm significance marks; hatched cells are non-significant.}
...
In Figure~\ref{fig:s10_power_sensitivity}, rows correspond to pairwise tx comparisons and columns to environments; signs and magnitudes are absolute PDR deltas, and cross markers denote Holm non-significant cells.
\begin{figure}[H]
\centering
...
The practical implication of the matched degradation result is that enabling the current patch configuration (MAC-collision plus multihop relay) reduces absolute AERIS PDR in all 24 matched cells, while relative ranking is preserved. This does not contradict the primary ranking result, because ranking and absolute-level behavior answer different questions: (i) ranking asks whether AERIS remains higher than alternatives under the same strict setting, while (ii) the matched stress-delta matrix asks how far the strict patch shifts each protocol relative to its own control path under identical sample sizes. In our data, strict-physics penalties affect all protocols, but the penalty profile is not uniform across protocol families. Therefore, the correct interpretation is not ``AERIS improves under patch'', but ``AERIS keeps relative advantages in harsh regimes while absolute reliability drops under stricter channel/relay assumptions.'' This distinction is central to deployment calibration.
