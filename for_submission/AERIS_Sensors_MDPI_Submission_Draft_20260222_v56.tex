...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{fig0_aeris_workflow_20260222_s56.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig1_env_pdr_panel_20260222_s56.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig2_ablation_panel_20260222_s56.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig3_scalability_panel_20260222_s56.pdf}
...
\subsection{Stress Block A: Patch-vs-Control (Four Environments)}
...
\begin{table}[H]
\caption{AERIS PDR in stress block A (patch vs control, four environments).}
...
Three observations are robust in stress block A. First, AERIS remains above LEACH/HEED/TEEN under patch mode in all four environments at 1000 nodes. Second, PEGASIS is minimally affected by the patch, with near-zero deltas and non-significant tests in all 24 PEGASIS cells\footnote{Across the 24 PEGASIS cells in stress block A, \(|\Delta_{\text{patch-control}}| \le 0.0044\), all Holm-adjusted \(p\)-values are \(1.0000\), and \(\max |g| = 0.0903\), consistent with a near-zero effect.}. Third, AERIS patch-control deltas are significant in all 24 AERIS cells after Holm correction, with the largest absolute drop in outdoor\_urban at high scale. Table~\ref{tab:s9_pegasis_1000} provides the 1000-node PEGASIS snapshot supporting this interpretation.

In stress block B, PEGASIS shows exact zero deltas in indoor\_factory (all six node counts). We treat this as an implementation-coupling anomaly pending code-path audit, not as physical invariance evidence. The patch can also be overly aggressive for baseline paths at high scale, so this block is interpreted as stress evidence rather than as a final replacement matrix. To remove the sample-size asymmetry in stress block A, we run stress block B as a matched confirmation block (patch \(n=1000\), control \(n=1000\) per cell) and report it separately below.

Consistency between stress block A and stress block B is assessed from the matched-delta tables and significance files; both blocks preserve the same direction pattern for AERIS across environment-scale cells.

\begin{table}[H]
\caption{PEGASIS patch-versus-control snapshot in stress block A at 1000 nodes (n=1000 patch, n=600 control).}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig6_s10_power_sensitivity_20260222_s56.pdf}
...
\subsection{Stress Block B: Matched Patch-vs-Control Matrix (Four Environments, n=1000 vs 1000)}
To remove remaining sample-size asymmetry from stress block A, we completed a fully matched stress block B matrix in which both patch and control arms use n=1000 per environment-node-protocol cell (4 environments $\times$ 6 node counts $\times$ 5 protocols). This block directly tests whether stricter physics assumptions improve or degrade reliability under matched sampling.

\begin{table}[H]
\caption{AERIS patch-control deltas in stress block B (delta = patch - control).}
...
Table~\ref{tab:s11_aeris_delta} summarizes the AERIS matched-delta boundary in stress block B.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig5_s11_patch_control_delta_20260222_s56.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig4_tradeoff_panel_20260222_s56.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig7_ns3_trend_panel_20260222_s56.pdf}
...
Stress block B shows that enabling the current patch configuration (\texttt{mac-collision + multihop-relay}) reduces absolute AERIS PDR in all 24 matched cells. This does not contradict the primary ranking result, because ranking and absolute-level behavior answer different questions: (i) ranking asks whether AERIS remains higher than alternatives under the same strict setting, while (ii) stress block B asks how far the strict patch shifts each protocol relative to its own control path. In our data, strict-physics penalties affect all protocols, but the penalty profile is not uniform across protocol families. Therefore, the correct interpretation is not ``AERIS improves under patch'', but ``AERIS keeps relative advantages in harsh regimes while absolute reliability drops under stricter channel/relay assumptions.'' This distinction is central to deployment calibration.
...
\item Stress block A, the tx-power sensitivity block, and stress block B show that stricter physics assumptions and tx-power changes can materially alter absolute PDR, so deployment requires environment-specific calibration.
\item In stress block B, PEGASIS shows near-invariant patch-control deltas; this behavior is treated as a simulator-coupling artifact pending dedicated code-path audit.
