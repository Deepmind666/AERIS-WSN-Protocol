...
This study reports two simulation regimes for scalability interpretation. The frozen S8 regime is the original simulator path used for the balanced 100--1000 node matrix. The S9/S10 regime introduces stricter physics controls (collision/relay stress and tx-power sensitivity) and is used for calibration and boundary checks. We therefore do not pool S8 and S9/S10 values into a single headline estimate. Instead, S8 is treated as the baseline reporting matrix, while S9/S10 are treated as rigor-calibration blocks that quantify how conclusions shift under stricter assumptions.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig1_env_pdr_panel_20260217_s25.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig2_ablation_panel_20260217_s25.pdf}
...
In the same S8 matrix, AERIS also shows a non-physical PDR-scale increase in three environments (indoor\_factory, outdoor\_urban, outdoor\_suburban). We therefore treat S8 as a frozen baseline regime under the original simulator assumptions and use S9/S10 as calibration evidence for physically stricter interpretation.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig3_scalability_panel_20260217_s25.pdf}
...
The S10 block confirms that power response is environment-dependent and non-monotonic under the current routing and channel settings. In particular, raising tx-power does not guarantee higher PDR in every environment-node cell. The single non-significant comparison in the 60-cell matrix is LEACH at indoor\_office, 1000 nodes.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig4_tradeoff_panel_20260217_s25.pdf}
...
\section{Discussion}
The evidence supports a scoped positioning rather than universal superiority. In the reported matrices, AERIS is strongest on reliability in harsh channels, while large-scale conclusions remain bounded to the tested baselines, environment taxonomy, and simulator assumptions.

The manuscript intentionally separates three evidence blocks:
\begin{itemize}
...
\end{itemize}
Cross-block numerical pooling is intentionally avoided because these blocks answer different questions. Under stricter S9 collision/relay settings, high-scale PDR decreases for most protocols and PEGASIS remains comparatively stable. Under S10, tx-power effects are strong but not globally monotonic. Therefore, practical interpretation should avoid one-dimensional claims such as ``higher power is always better'' and should instead remain environment-scoped.
...
\item For high-scale deployments, treat S8 as baseline evidence and use S9/S10 calibration before final engineering parameter selection.
...
\midrule
indoor\_office & Reliability differences are small at scale; optimize for deployment cost first & Several cells are non-significant in NS-3 trend checks \\
indoor\_factory & AERIS/Gateway gains are most stable and practically relevant & Validate tx-power and collision settings before deployment \\
outdoor\_urban & AERIS remains preferable under harsh links & High-scale behavior is sensitive to physics assumptions \\
outdoor\_suburban & AERIS trend remains favorable in both simulator and NS-3 trend blocks & Avoid monotonic tx-power assumptions \\
\bottomrule
\end{tabular}
\normalsize
\end{table}
...
\item The large-scale S8 matrix is balanced at n=1000 per cell, but claims remain bounded to the tested channel taxonomy and original simulator assumptions.
...
\section{Conclusion}
AERIS provides a reliability-oriented routing option for heterogeneous WSN channels within clearly bounded evidence regimes. In the publication-tier 100-node matrix, it achieves the highest mean PDR within the tested baseline set across all four environments. In the balanced S8 large-scale matrix (100--1000 nodes, n=1000 per cell), AERIS is the highest-mean protocol in all tested environments and scales under the original simulator setting; however, this block is treated as bounded baseline evidence because it excludes MAC-collision modeling and exhibits non-physical upward trends in three environments. NS-3 provides trend-level support for AERIS versus LEACH and is not used as a numerical-equivalence proof. The completed S9 stress test and S10 power-sensitivity matrix jointly show that stricter physics and tx-power changes can materially alter high-scale behavior. Therefore, collision/relay parameter calibration remains necessary before replacing S8 with a finalized camera-ready scalability matrix. Gateway effects are environment-dependent and CAS effects are mixed; all conclusions are explicitly constrained to the tested protocol set, environment taxonomy, metric definition, and sample-size regimes.
