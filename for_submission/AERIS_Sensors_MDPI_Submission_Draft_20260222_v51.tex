...
\abstract{This paper presents AERIS, an environment-aware hierarchical routing protocol for wireless sensor networks, and evaluates it with \texttt{pdr\_expected}. In a four-environment 100-node matrix (30 seeds per environment), AERIS provides the highest mean PDR among LEACH, PEGASIS, HEED, and TEEN. In a stricter large-scale matrix (100--1000 nodes, 3200 replicates per environment-node-protocol cell, explicit MAC-collision and multi-hop relay enabled), AERIS ranks first in 18 of 24 environment-scale cells. The rank-1 cells cover all six scales in indoor\_factory, outdoor\_urban, and outdoor\_suburban; PEGASIS remains stronger in indoor\_office at high scale. A matched tx-power sensitivity block (5 vs 15 dBm, n=600 per cell) shows significant environment-dependent responses in 59/60 cells after Holm correction. A matched patch-control block (n=1000 vs 1000 per cell) shows negative AERIS deltas in all 24 cells, indicating that the current strict-physics patch imposes measurable reliability costs. NS-3 is used for cross-platform trend validation (AERIS vs LEACH), and all NS-3 claims are reported as trend-level rather than numerical-equivalence evidence.}
...
\item publication-tier evidence matrices that separate core reliability reporting (100-node + v50-rigor scalability matrix) from stress/sensitivity calibration blocks (S9/S10/S11), avoiding cross-regime over-aggregation;
...
The CAS coefficients $\{w_{1,m}, w_{2,m}, w_{3,m}, b_m\}$ are fixed per environment family from an offline grid search and then held constant across all publication-tier runs.
...
Let $N$ be the node count and $H$ be the CH count in one round. CH election and clustering are approximately $O(NH)$ in the current implementation, CAS selection is $O(1)$ per cluster decision, and Gateway ranking is $O(H\log H)$ when sorting candidates. Therefore, the dominant per-round cost is approximately
\begin{equation}
T_{\mathrm{round}} = O(NH + H\log H),
\end{equation}
where $H \ll N$ in the tested regimes. This complexity level is consistent with lightweight distributed WSN routing control.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.97\textwidth]{fig0_aeris_workflow_20260222_s50.pdf}
...
\item Seeds for scalability: v50-rigor authoritative regime with n=3200 per environment-node-protocol cell (100--1000 nodes, 4 environments; strict-physics flags enabled).
...
To reduce implementation-bias risk, v50-rigor removes previously identified baseline suppressors and reruns the four-environment scalability matrix under explicit collision and relay settings. NS-3 is then used as an independent trend-level check for AERIS versus LEACH under aligned environment definitions. We treat NS-3 as external directional validation, not as a one-to-one numerical replacement of the Python matrix.

\subsection{Simulator Regimes and Physics Assumptions}
This study reports two regime tiers for scalability interpretation. The primary tier is the v50-rigor matrix (explicit \texttt{--mac-collision --multihop-relay}, balanced n=3200 per cell) and is used for large-scale ranking claims. The secondary tier (S9/S10/S11) is retained for stress and sensitivity diagnostics (patch-vs-control and tx-power response), not for replacing the primary v50 ranking matrix.
...
v50-rigor scalability & 4 env $\times$ 6 node counts $\times$ 5 protocols & n=3200 per env-node-protocol cell & Primary large-scale matrix \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig1_env_pdr_panel_20260222_s50.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig2_ablation_panel_20260222_s50.pdf}
...
\begin{table}[H]
\caption{PDR ranking at 1000 nodes from the v50-rigor matrix (n=3200 per cell).}
\label{tab:scale1000}
\centering
...
AERIS is first in three environments at 1000 nodes (indoor\_factory, outdoor\_urban, outdoor\_suburban) and second in indoor\_office where PEGASIS remains dominant.
...
Unlike the legacy S8 path, the v50-rigor matrix shows physically plausible scale behavior: AERIS PDR decreases from 100 to 1000 nodes in all four environments under explicit collision and relay constraints.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig3_scalability_panel_20260222_s50.pdf}
\caption{v50-rigor scalability trends over node counts (balanced n=3200 per environment-node-protocol cell) with 95\% CI bands. Explicit collision and relay modeling are enabled. AERIS remains rank-1 in three environments and declines with scale in all four environments.}
\label{fig:scalability_panel}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig8_s8_significance_heatmap_20260222_s50.pdf}
\caption{v50-rigor significance map against the strongest baseline per environment-node cell. Panel (a) reports signed delta (percentage points) with zero-centered color normalization, and panel (b) reports effect-size strength as $|g|$ (Hedges' $g$) with Holm significance markers and p-value annotations.}
\label{fig:s8_significance_map}
\end{figure}

Figures~\ref{fig:scalability_panel} and~\ref{fig:s8_significance_map} jointly provide the v50-rigor trend-and-significance view.
...
The key pilot observation is monotonicity recovery: AERIS PDR decreases with scale in all four environments. In indoor\_office, PEGASIS is higher than AERIS under this stricter setting, while AERIS remains clearly higher in the other three environments. The pilot therefore served as a gate check for launching the full v50-rigor matrix and is reported only for diagnostic completeness.
...
Table~\ref{tab:s9_patch_control} provides the matched S9 patch-control baseline for cross-regime interpretation.

Three observations are robust in S9. First, AERIS remains above LEACH/HEED/TEEN under patch mode in all four environments at 1000 nodes. Second, PEGASIS is minimally affected by the patch, with near-zero deltas and non-significant tests in all 24 PEGASIS cells\footnote{Across the 24 PEGASIS cells in the S9 matched matrix, \(|\Delta_{\text{patch-control}}| \le 0.0044\), all Holm-adjusted \(p\)-values are \(1.0000\), and \(\max |g| = 0.0903\), consistent with a near-zero effect.}. Third, AERIS patch-control deltas are significant in all 24 AERIS cells after Holm correction, with the largest absolute drop in outdoor\_urban at high scale. Table~\ref{tab:s9_pegasis_1000} provides the 1000-node PEGASIS snapshot supporting this interpretation.

In S11, PEGASIS shows exact zero deltas in indoor\_factory (all six node counts). We treat this as an implementation-coupling anomaly pending code-path audit, not as physical invariance evidence. The patch can also be overly aggressive for baseline paths at high scale, so this block is interpreted as stress evidence rather than as a final replacement matrix. To remove the remaining S9 sample-size asymmetry, we run S11 as a matched confirmation block (patch \(n=1000\), control \(n=1000\) per cell) and report it separately below.

S9/S11 consistency is assessed from the matched-delta tables and significance files; both blocks preserve the same direction pattern for AERIS across environment-scale cells.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig6_s10_power_sensitivity_20260222_s50.pdf}
...
Figure~\ref{fig:s10_power_sensitivity} is interpreted together with Table~\ref{tab:s10_aeris_1000} to avoid delta-only bias.
...
Table~\ref{tab:s11_aeris_delta} summarizes the AERIS matched-delta boundary in S11.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig5_s11_patch_control_delta_20260222_s50.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig11_s11_significance_panel_20260222_s50.pdf}
\caption{S11 significance density and effect-size map. Panel (a) reports AERIS Hedges' $g$ by environment and node count for patch-control differences. Panel (b) reports the number of Holm-significant cells per protocol (out of 24).}
\label{fig:s11_significance_panel}
\end{figure}

Figures~\ref{fig:s11_patch_control} and~\ref{fig:s11_significance_panel} together show both absolute deltas and significance density in S11.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig4_tradeoff_panel_20260222_s50.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig7_ns3_trend_panel_20260222_s50.pdf}
...
\item large-scale v50-rigor regime (100--1000 nodes) with balanced n=3200 per cell,
...
\subsection*{Interpretation of S11 Degradation}
S11 shows that enabling the current patch configuration (\texttt{mac-collision + multihop-relay}) reduces absolute AERIS PDR in all 24 matched cells. This does not contradict the v50-rigor ranking result, because ranking and absolute-level behavior answer different questions: (i) ranking asks whether AERIS remains higher than alternatives under the same strict setting, while (ii) S11 asks how far the strict patch shifts each protocol relative to its own control path. In our data, strict-physics penalties affect all protocols, but the penalty profile is not uniform across protocol families. Therefore, the correct interpretation is not ``AERIS improves under patch'', but ``AERIS keeps relative advantages in harsh regimes while absolute reliability drops under stricter channel/relay assumptions.'' This distinction is central to deployment calibration.
...
\item For high-scale deployments, use the v50-rigor matrix as primary evidence and use S9/S10/S11 for stress and sensitivity calibration.
...
\item The large-scale v50-rigor matrix is balanced at n=3200 per cell and includes explicit collision and relay constraints, but remains simulation evidence.
...
\item Under patch mode, high-scale baseline PDR can become very low in benign channels, so collision/relay parameter calibration remains required for deployment tuning.
...
\section{Conclusion}
AERIS is a reliability-oriented routing option for heterogeneous WSN channels. In the publication-tier 100-node matrix, it achieves the highest mean PDR within the tested baseline set across all four environments. In the v50-rigor large-scale matrix (100--1000 nodes, n=3200 per cell, explicit collision/relay modeling), AERIS is rank-1 in three environments (indoor\_factory, outdoor\_urban, outdoor\_suburban) and rank-2 in indoor\_office where PEGASIS remains dominant.

NS-3 provides trend-level cross-platform support for AERIS versus LEACH. The completed S9 stress block, S10 power-sensitivity matrix, and S11 matched patch-control matrix show that stricter physics assumptions and tx-power changes materially affect high-scale behavior. In S11, AERIS patch-control deltas are negative and significant in all 24 environment-scale cells under the current patch setup.
