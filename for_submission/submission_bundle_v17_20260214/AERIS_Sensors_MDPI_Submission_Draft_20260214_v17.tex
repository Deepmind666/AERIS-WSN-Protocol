...
\item Seeds for scalability: mixed frozen regime with n=550 in indoor office, outdoor urban, and outdoor suburban, and n=650 in indoor factory.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig1_env_pdr_panel_20260214_s22.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig2_ablation_panel_20260214_s22.pdf}
...
\begin{table}[H]
\caption{PDR ranking at 1000 nodes (mixed sample sizes: n=650 in indoor\_factory, n=1000 in other environments).}
\label{tab:scale1000}
\centering
...
AERIS is first in three environments in this mixed frozen large-scale matrix, with indoor\_office as a stable counterexample where PEGASIS is higher. This statement is regime-specific and must not be interpreted as a pooled cross-environment ranking.
...
\begin{table}[H]
\caption{Representative Holm-corrected significance snapshot from the 1000-node scalability matrix (AERIS vs LEACH).}
...
\midrule
indoor\_office    & 1000 & -0.000286 & -0.2673 & 1.00 & no \\
...
This snapshot highlights the scoped conclusion used throughout this manuscript: strong and consistent gains in harsh environments, with indoor\_office showing a non-significant counterexample at 1000 nodes.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig3_scalability_panel_20260214_s22.pdf}
\caption{Scalability trends over node counts (indoor\_factory n=650; indoor\_office, outdoor\_urban, and outdoor\_suburban n=550) with 95\% CI bands. The indoor\_office panel uses a narrowed y-axis window to expose non-zero ordering gaps near PDR=1.0.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig4_tradeoff_panel_20260214_s22.pdf}
...
\section{Discussion}
The evidence supports a scoped positioning rather than universal superiority. In the current reported matrices, AERIS is strong for reliability across heterogeneous channels, while all large-scale claims remain bounded to environment-specific sample-size regimes and tested baselines.
...
\section{Conclusion}
AERIS provides a reliability-oriented routing option for heterogeneous WSN channels. In the publication-tier 100-node matrix, it achieves the highest mean PDR within the tested baseline set across all four environments. In the current large-scale frozen matrix, it has the highest mean PDR in three of four environments, with indoor\_office as a stable counterexample where PEGASIS is higher. NS-3 confirms trend consistency for AERIS versus LEACH and is used strictly as trend-level evidence. Gateway effects are environment-dependent and CAS effects are mixed; therefore, the final claim is explicitly bounded to the tested protocol set, environment taxonomy, metric definition, and sample-size regimes.
