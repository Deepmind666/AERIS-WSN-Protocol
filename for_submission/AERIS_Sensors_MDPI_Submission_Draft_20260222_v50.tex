...
\abstract{This paper presents AERIS, an environment-aware hierarchical routing protocol for wireless sensor networks, and evaluates it using \texttt{pdr\_expected} as the primary metric. In the publication-tier 100-node matrix (four environments, 30 seeds per environment), AERIS achieves the highest mean PDR among LEACH, PEGASIS, HEED, and TEEN. In the v50-rigor authoritative scalability matrix (100--1000 nodes, four environments, 3200 replicates per environment-node-protocol cell, explicit MAC-collision and multi-hop relay enabled), AERIS ranks first in 18/24 environment-scale cells and in all six node scales for indoor\_factory, outdoor\_urban, and outdoor\_suburban; indoor\_office is PEGASIS-dominant at high scale. In S10 (tx-power 5 vs 15 dBm, n=600 per cell), 59/60 comparisons are significant after Holm correction. In matched S11 (patch n=1000, control n=1000), AERIS patch-control deltas are negative and significant in all 24 environment-scale cells, indicating reliability degradation under the current collision/relay patch configuration. NS-3 is used for cross-platform trend validation (AERIS vs LEACH) and is reported as trend-level evidence rather than numerical-equivalence proof.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig9_s9_s11_consistency_20260222_s50.pdf}
\caption{Consistency check between S9 and S11 patch-control deltas. Panel (a) shows cell-wise agreement across all protocols and scales; panel (b) shows AERIS trajectories by node count under S9 and S11.}
\label{fig:s9_s11_consistency}
\end{figure}

Figure~\ref{fig:s9_s11_consistency} shows that S9 and S11 produce consistent delta-direction behavior.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig10_s10_absolute_profiles_20260222_s50.pdf}
\caption{Absolute S10 profiles under tx5 (dashed) and tx15 (solid) at node counts 100/500/1000. This view complements the delta map by showing regime-level direction reversals directly on PDR scale.}
...
Figures~\ref{fig:s10_power_sensitivity} and~\ref{fig:s10_absolute_profiles} are interpreted together to avoid delta-only bias.
