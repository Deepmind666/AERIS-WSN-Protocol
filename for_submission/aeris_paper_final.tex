...
This paper introduces AERIS (Adaptive Environment-aware Routing with Intelligent Switching), a protocol that achieves \textbf{99.2\% PDR at 100 nodes and maintains 79.7\% at 500 nodes} under Log-Normal Shadowing channels, while classical protocols degrade significantly. The key insight is that gateway-enhanced relay with safety thresholds provides more robust packet delivery than chain-based or cluster-only approaches under realistic propagation conditions.

Our contributions are:

\begin{enumerate}
    \item \textbf{Environment-Aware Protocol Design}: We present AERIS, which integrates environment-link correlation analysis (humidity-link $r=-0.499$) with threshold-based relay switching, achieving 99.2\% PDR at 100 nodes (79.7\% at 500 nodes) where classical protocols achieve only 39--88\% under identical conditions.
    ...
    \item \textbf{Scalability Advantage}: AERIS maintains high PDR from 100 nodes (99.2\%) to 500 nodes (79.7\%), while LEACH degrades from 68.7\% to 43.7\% (25.0 pp drop) and PEGASIS from 88.4\% to 54.2\% (34.2 pp drop). This demonstrates AERIS's robustness at scale.
...
% - AERIS achieves 99.2% PDR at 100 nodes, 79.7% at 500 nodes
% - LEACH: 68.7%$\rightarrow$43.7%, PEGASIS: 88.4%$\rightarrow$54.2%, HEED: 71.7%$\rightarrow$39.7%
...
\begin{figure}[H]
    ...
    \textbf{(a)} PDR comparison: AERIS achieves 99.2\% PDR at 100 nodes, while LEACH (68.7\%) and PEGASIS (88.4\%) show lower performance.
    \textbf{(b)} Total energy consumption (mJ); AERIS consumes 81.4~mJ at 100 nodes.
    \textbf{(c)} PDR-Energy trade-off analysis.
    \textbf{(d)} Scalability analysis (100--500 nodes); AERIS maintains 79.7\% PDR at 500 nodes while baselines degrade to 39--54\%.
...
Figure~\ref{fig:sota} presents a comprehensive comparison against five baseline protocols (LEACH/HEED/PEGASIS/SEP/TEEN) together with the two AERIS profiles (AERIS-E and AERIS-R). All protocols are evaluated under identical geometry, energy, and channel models to ensure a fair comparison. The statistical testing procedure follows the recommended workflow: Shapiro--Wilk normality test, Levene variance homogeneity test, followed by selection of the appropriate statistical test (independent t-test or Mann--Whitney U test).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{sota_comparison_6panel.pdf}
    \caption{\textbf{Protocol comparison under Log-Normal Shadowing ($\sigma=8$~dB, $n=30$ runs each).}
    \textbf{(a)} PDR comparison (mean $\pm$ 95\% CI): AERIS-R (99.93\%) and AERIS-E (99.09\%) are highest; PEGASIS follows (96.38\%); LEACH/HEED/SEP cluster near 87--89\%; TEEN is lowest (57.53\%).
    \textbf{(b)} PDR distribution box plots.
    \textbf{(c)} $\Delta$PDR vs baselines for AERIS-E/R (mean $\pm$ 95\% CI).
    \textbf{(d)} PDR--energy trade-off with per-run scatter and mean $\pm$ 95\% CI.
    \textbf{(e)} Energy consumption (mean $\pm$ SD): TEEN is lowest (15.24~J); LEACH/HEED/SEP/PEGASIS are near 18--19~J; AERIS-E/R are highest (21.16--21.31~J).
    \textbf{(f)} Statistical validation summary (Shapiro--Wilk $\rightarrow$ Levene $\rightarrow$ t-test).}
    \label{fig:sota}
\end{figure}

The principal findings from the SOTA comparison at 100 nodes:
\begin{itemize}
    \item \textbf{PDR Performance}: AERIS-R/E achieve near-100\% PDR (99.93\% / 99.09\%). PEGASIS is next (96.38\%); LEACH/HEED/SEP cluster near 87--89\%; TEEN is lowest (57.53\%).
    \item \textbf{Energy Trade-off}: TEEN is most energy-frugal (15.24~J) but unreliable. Baselines cluster near 18--19~J, while AERIS-E/R consume more energy (21.16--21.31~J) to reach higher PDR.
    \item \textbf{Effect sizes}: AERIS-R shows large positive $\Delta$PDR vs LEACH/HEED/SEP/TEEN; PEGASIS remains a strong baseline in PDR with lower energy than AERIS.
\end{itemize}

\textbf{Honest Assessment}: AERIS-R delivers the highest PDR at an energy premium; AERIS-E is more balanced but still above classical baselines in PDR. PEGASIS remains a strong baseline with lower energy than AERIS in this 100-node SOTA setting. The appropriate protocol choice depends on application requirements and deployment constraints.
...
\begin{itemize}
    \item \textbf{AERIS}: Achieves 99.2\% PDR at 100 nodes, maintaining 79.7\% even at 500 nodes
    \item \textbf{Classical Protocols}: Show significant degradation under realistic channels:
    \begin{itemize}
        \item LEACH: 68.7\% (100 nodes) $\rightarrow$ 43.7\% (500 nodes)
        \item PEGASIS: 88.4\% (100 nodes) $\rightarrow$ 54.2\% (500 nodes)
        \item HEED: 71.7\% (100 nodes) $\rightarrow$ 39.7\% (500 nodes)
    \end{itemize}
\end{itemize}
...
\textbf{(2) Single-Hop Limitations in LEACH.} LEACH cluster heads transmit directly to the base station. Under shadowing, distant CHs experience severe signal attenuation. Without relay mechanisms, packets from far clusters are frequently lost. LEACH PDR degrades from 68.7\% (100 nodes) to 43.7\% (500 nodes) as network area increases.

\textbf{(3) HEED's Coverage Gaps.} HEED's residual-energy-weighted CH selection optimizes for energy balance but not for coverage. Under realistic channels, some regions lack reliable paths to the BS. HEED PDR degrades from 71.7\% (100 nodes) to 39.7\% (500 nodes).

\textbf{(4) AERIS's Gateway-Enhanced Solution.} AERIS addresses these limitations through: (a) Gateway nodes providing relay infrastructure for CH-to-BS communication, (b) Safety mechanism enabling cooperative fallback when primary paths fail, and (c) ARQ retry ensuring packet delivery even under transient link failures. This combination achieves 99.2\% PDR at 100 nodes and maintains 79.7\% at 500 nodes.
...
\begin{itemize}
    \item \textbf{At 100 nodes}: AERIS achieves 99.2\% PDR, compared to LEACH (68.7\%), PEGASIS (88.4\%), HEED (71.7\%)
    \item \textbf{At 300 nodes}: AERIS maintains 83.5\% PDR, while LEACH degrades to 52.0\%, PEGASIS to 66.4\%, HEED to 51.5\%
    \item \textbf{At 500 nodes}: AERIS achieves 79.7\% PDR, while LEACH falls to 43.7\% (36.0 pp gap), PEGASIS to 54.2\% (25.5 pp gap), HEED to 39.7\% (40.0 pp gap)
\end{itemize}

The performance gap remains significant across all scales. At 100 nodes, AERIS outperforms LEACH by 30.5 percentage points; at 500 nodes, this gap is 36.0 percentage points. This demonstrates AERIS's superior scalability under realistic channel conditions.
...
\textbf{Key Insight}: Classical protocols achieve near-optimal performance under idealized channel assumptions but lack mechanisms to handle realistic propagation effects. AERIS's design explicitly accounts for stochastic channel variations, achieving 99.2\% PDR at 100 nodes (79.7\% at 500 nodes) where classical protocols achieve only 39--88\%.
...
\begin{itemize}
    \item \textbf{Reliability}: 99.2\% PDR at 100 nodes, maintaining 79.7\% at 500 nodes, compared to 39--88\% for classical protocols
    \item \textbf{Scalability}: Performance advantage maintained at larger scales---at 500 nodes, AERIS outperforms LEACH by 36.0 percentage points
...
\textbf{Energy Trade-off}: AERIS consumes more energy than PEGASIS (806.9~mJ vs 368.2~mJ at 500 nodes). This is the cost of 79.7\% reliability versus PEGASIS's 54.2\%. Applications prioritizing reliability over energy efficiency should choose AERIS; energy-constrained applications tolerating packet loss may prefer PEGASIS.
...
\begin{enumerate}
    \item \textbf{Superior PDR under realistic channels}: AERIS achieves 99.2\% Packet Delivery Ratio at 100 nodes, maintaining 79.7\% at 500 nodes, significantly outperforming classical protocols: LEACH (68.7\%$\rightarrow$43.7\%), PEGASIS (88.4\%$\rightarrow$54.2\%), HEED (71.7\%$\rightarrow$39.7\%).
    ...
    \item \textbf{Scalability advantage}: AERIS maintains significant performance advantage at larger scales. At 500 nodes, AERIS outperforms LEACH by 36.0 percentage points, demonstrating superior scalability.
    ...
    \item \textbf{Honest energy trade-off}: AERIS consumes more energy than PEGASIS (806.9~mJ vs 368.2~mJ at 500 nodes) but achieves 79.7\% reliability versus PEGASIS's 54.2\%. The energy cost is the price of higher delivery rate.
