...
\abstract{This study evaluates AERIS, a lightweight hierarchical routing protocol for wireless sensor networks under heterogeneous channel conditions, using \texttt{pdr\_expected} as the primary metric. In the publication-tier 100-node regime (four environments, 30 seeds per environment), AERIS achieves the highest mean PDR within the tested baseline set (LEACH, PEGASIS, HEED, and TEEN). In the frozen S8 scalability matrix (100--1000 nodes, four environments, 1000 replicates per environment-node-protocol cell), AERIS remains the highest-mean protocol in all 24 environment-scale cells; however, this block is treated as baseline-only evidence because explicit MAC-collision modeling is absent and non-physical upward trends appear in three environments. Accordingly, S9/S10/S11 are used as stricter-physics calibration blocks. S10 (tx-power 5 vs 15 dBm, n=600 per cell) shows significant differences in 59/60 cells after Holm correction (single non-significant case: LEACH at indoor\_office, 1000 nodes). In the fully matched S11 patch-versus-control matrix (n=1000 in both arms), AERIS patch-control deltas are negative and significant in all 24 environment-scale cells, indicating reliability degradation under the current collision/relay patch configuration. NS-3 is used for trend-level cross-platform validation (AERIS vs LEACH), not as a numerical-equivalence proof.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig1_env_pdr_panel_20260219_s37.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig2_ablation_panel_20260219_s37.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig3_scalability_panel_20260219_s37.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig5_s11_patch_control_delta_20260219_s37.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig4_tradeoff_panel_20260219_s37.pdf}
