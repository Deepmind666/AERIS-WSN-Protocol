...
\item A rigor-patch pilot (collision and baseline relay enabled) changes benign-environment ranking and restores monotonic scale behavior; a full rerun under this upgraded setting is still pending.
...
\section{Conclusion}
AERIS provides a reliability-oriented routing option for heterogeneous WSN channels. In the publication-tier 100-node matrix, it achieves the highest mean PDR within the tested baseline set across all four environments. In the current balanced large-scale frozen matrix (100--1000 nodes, n=1000 per cell), AERIS remains the highest-mean protocol in all tested environments and scales. NS-3 confirms trend consistency for AERIS versus LEACH and is used strictly as trend-level evidence. The latest rigor-patch pilot indicates that stricter simulator physics can alter benign-environment ordering while preserving AERIS strength in harsh channels, so final camera-ready scalability claims should be revalidated after a full rerun under the upgraded settings. Gateway effects are environment-dependent and CAS effects are mixed; therefore, all conclusions remain explicitly bounded to the tested protocol set, environment taxonomy, metric definition, and sample-size regimes.
