...
\subsection{Matched S9 Patch-vs-Control Stress Test (Two Environments)}
To reduce seed and sampling confounders, we additionally ran a matched patch-versus-control stress test in two environments (indoor\_office and outdoor\_suburban). The patch mode enables collision and baseline relay mechanisms; the control mode uses the original simulator path. Patch cells use n=1000 and control cells use n=600.

\begin{table}[H]
\caption{AERIS PDR in matched S9 stress test (patch vs control).}
...
Two observations are robust in the current S9 evidence: (i) AERIS remains substantially above LEACH/HEED/TEEN under patch mode in both tested environments; (ii) PEGASIS is minimally affected by the patch, with near-zero deltas and non-significant tests in all 12 PEGASIS cells. At the same time, the patch can be overly aggressive for some baseline paths at high scale, so this block is interpreted as a stress-test result rather than a finalized replacement matrix. The remaining two environments (indoor\_factory and outdoor\_urban) are pending in the same matched patch-versus-control regime.
...
\item A matched S9 patch-versus-control stress test has been completed for indoor\_office and outdoor\_suburban; indoor\_factory and outdoor\_urban are still pending in the same regime.
