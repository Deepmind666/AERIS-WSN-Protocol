...
## 2026-01-23 更新：6-panel 复查与修正（二次）

### 本次修正点
1. **(c) 面板表达澄清**  
   - 改为清晰的“ΔPDR (AERIS profile − protocol, percentage points)”标注。  
   - 保留 0 基准虚线，显示 AERIS-E 与 AERIS-R 两条对比点（每基线 2 个点）。
2. **(d) 面板信息不足问题**  
   - 改为“Runs + Mean ± 95% CI”，同时展示每次运行散点与均值误差棒。  
   - 避免“直线造假”观感，体现实验分布与离散性。
3. **(f) 表格可读性**  
   - 字号放大（10.6），表格整体缩放增大（1.30 × 1.80）。  
   - 保留 ΔPDR-E/ΔPDR-R 与对应 p 值，颜色区分正负差异。
4. **整体排版**  
   - 画布宽度增大，右侧面板更宽，减轻遮挡与拥挤感。

### 当前仍需严格注意的风险
- **TEEN 语义差异**：事件驱动协议仍可能导致 PDR 不可直接对比，正文需强调比较边界。  
- **AERIS-E 可靠性不足**：PDR 显著低于多基线，定位必须清晰为“节能优先配置”。  
- **(d) 面板数据密度**：虽然已加入散点，但若缩放后仍拥挤，需进一步减少标注或压缩图例。

### 结论（是否可用于论文）
- 该图现已达到“可用于论文”水平，但依赖于正文解释：
  1) AERIS-R 作为可靠性提升配置；
  2) AERIS-E 作为能耗优先配置；
  3) TEEN 为事件驱动协议，PDR 对比需谨慎解读。
